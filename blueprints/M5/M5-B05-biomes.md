...
**Pure-function consequence (GEN-D25/D26).** Vanilla's own biome selection is `(seed-derived density-function output, coordinates) -> biome` with no RNG only for tie-free targets in a freshly generated, non-blended, non-upgrading world; the real algorithm carries three exclusions from that ideal — the R-tree-equivalent search's own thread-local last-successful-leaf cache seeds each query and can decide an exact fitness tie by that thread's own query history (Context §E), `Blender`'s biome resolver can override the result with a neighbouring old chunk's stored biome while blending, and `BelowZeroRetrogen`'s resolver can replace it with the proto-chunk's own already-stored biome while a pre-1.18 chunk is upgrading. This blueprint implements none of the three: `find_biome_naive` and `BiomeSearchTree` (Context §E) are cold-started on every call, and neither `MultiNoiseBiomeSource` nor `fill_biome_column` ever wraps its result through a blending or retrogen resolver (that wiring, if it is ever needed, is a future chunk-generation blueprint's job, not this one's). This blueprint's own deliverables are therefore, by construction, a pure function of `(sampler output, coordinates)` with no RNG, no cross-chunk read, and no cached mutable state of their own beyond the noise router's own memoization (owned by M5-B03, not this blueprint) — safe to call from any worker, any interleaving, any number of times, with byte-identical results. This blueprint introduces no locking, no shared mutable cache, and no ordering constraint of its own.
...
- `BiomeParameterList { entries: Vec<(QuantizedClimatePoint, ResourceLocation)> }` — `entries` preserves the **stored order** of the source JSON/report file's own flat entry list (M5-B02 Deliverables' `RawWorldgenJson.biome_parameter_lists: BTreeMap<ResourceLocation, Vec<BiomeParameterEntryJson>>` comment: "= that file's flat entry list" — a `Vec`, never re-sorted by `compile()`), which is in turn the codec-decoded list order, or a built-in preset's own emission order (`MultiNoiseBiomeSourceParameterList.java:56-64` for the nether, `OverworldBiomeBuilder.addBiomes` for the overworld) — **list order, not registry-load order**, and notably not the order vanilla's own production search actually visits entries in (Context §E: the real R-tree-equivalent structure re-sorts children at every level of `build`; only its `@VisibleForTesting`-only brute-force scan ever reads the stored list's own order).
...
**Naive search** — a strict linear scan over `BiomeParameterList.entries` in their **stored** order (§B — vanilla's own stored list order, the same order only vanilla's `@VisibleForTesting` `findValueBruteForce` ever scans; vanilla's actual production search never reads this order, always going through the rebuilt R-tree-equivalent structure instead), first-strictly-better wins on ties (`<`, never `<=`):
...
26.2 does have a climate-driven spawn-point search: `Climate.findSpawnPosition` (`17-noise-math.md` §3.9). It searches against `NoiseGeneratorSettings.spawn_target` (compiled by M5-B02 as `Vec<QuantizedClimatePoint>`, typically a handful of entries — a **separate, small list**, never the full `BiomeParameterList`) via an Archimedean-spiral radial walk, ranking candidates by `min_fitness_over_spawn_targets * 2048² + (blockX² + blockZ²)` (climate fitness dominates unless two candidates are within `1/2048²` "worth" of origin-distance of each other), with the queried climate point's own `depth` axis forced to `0` before computing fitness (`zeroDepthTargetPoint`) and `x`/`z` truncated (not rounded) from `sin`/`cos(angle)·radius`. Each `radialSearch` pass initializes `radius` to that pass's own `radiusIncrement` (never `0`) and loops while `radius <= maxRadius`, advancing the angle by `radiusIncrement / radius` each step and, only once the angle passes a full `2π` revolution, resetting the angle to `0` and bumping `radius` by `radiusIncrement` — so the first pass visits radii `512, 1024, 1536, 2048` only, and the second, re-centered on the first pass's own best-found result, visits `32` up to and including `512`; the origin itself is scored exactly once, before either pass, and is never re-scored inside one. This blueprint's own best-effort, explicitly-flagged reconstruction:
...
fn find_spawn_position(sampler, spawn_targets) -> (i32, i32):
    ...
    fn radial_pass(center_x, center_z, radius_increment, max_radius, mut best) -> (i32,i32,i64):
        radius = radius_increment; angle = 0.0f32
        while radius <= max_radius:
            bx = center_x + (angle.sin() * radius) as i32
            bz = center_z + (angle.cos() * radius) as i32
            s = score(bx, bz)
...
The `radialSearch` loop shape above (radius initialization at `radiusIncrement`, the `radius <= maxRadius` guard, and the angle-wrap-then-radius-bump termination) is confirmed against the reference (TEST-D57); this section is nonetheless still marked moderate-confidence overall, since the compiled `spawn_target` list this blueprint's own data pipeline extracts and the cross-JDK `sin`/`cos` ordering hazard (`docs/research/mc-26.2/18-float-determinism.md` §3.7) remain outside what this restatement alone can settle. A future GEN-D27 differential run against the real vanilla server's own spawn point is the actual reconciliation step — this blueprint's own acceptance test for this function (§Acceptance tests) checks structural properties (determinism, staying within the documented search radius) rather than a golden coordinate vector it cannot honestly claim to have verified.
...
- Vanilla's overworld and nether dimensions use the identical multi-noise biome-placement algorithm, differing only in which compiled biome-parameter-list data is supplied to it.
- Nether's biome-parameter list contains 5 entries.
- Nether's biome-parameter-list entries pin the depth and weirdness axes as quantized point spans with min 0 and max 0 on all five entries, rather than omitting them; the full-range -10000..10000 wildcard span exists only on the Overworld side.
- A wildcard span whose range fully covers the target's value on that axis always contributes a distance, and therefore a squared distance, of 0.
- The End dimension's biome placement uses a structurally different, hardcoded (non-JSON-driven, non-R-tree) algorithm (TheEndBiomeSource): a central 64-chunk (1024-block)-radius island special case plus threshold-bucketed reuse of the erosion axis slot.
- The End dimension has no multi-noise biome-parameter list at all.
- Vanilla's biome-selection functions (Climate.Sampler.sample, Climate.ParameterList.findValue) and its spawn-point search (Climate.findSpawnPosition) are pure functions of density-function output and quantized-integer arithmetic, with no Random/RandomSource reference anywhere in either algorithm.
- Vanilla's biome selection at a given coordinate depends only on that coordinate's own seed-derived density-function output, with no cross-chunk read and no cached mutable state of its own beyond the noise router's own memoization, except for three exclusions: the R-tree-equivalent search's own thread-local last-result cache can decide an exact fitness tie by that thread's query history, Blender's biome resolver can override the result with a neighbouring old chunk's stored biome while blending, and BelowZeroRetrogen's resolver can replace it with the proto-chunk's own stored biome while a pre-1.18 chunk is upgrading; the claim holds for tie-free targets in a freshly generated, non-blended, non-upgrading world.
- A biome-parameter list's entries are stored in the codec-decoded list order (or a built-in preset's own emission order), a Vec never re-sorted by that storage step itself, but vanilla's production search never scans that stored list at all: findValue always delegates to the rebuilt R-tree-equivalent search structure, which re-sorts children at every level of build, and only its @VisibleForTesting-only brute-force scan visits entries in stored order.
- quantize_climate is computed as (v * 10000.0) as i64 for an input f32 v.
- Vanilla's NoiseRouter climate-target density functions are six fields named temperature, vegetation, continents, erosion, depth, and ridges.
- Of those six NoiseRouter fields, three field names differ from the climate axis they actually feed: vegetation feeds the humidity axis, continents feeds the continentalness axis, and ridges feeds the weirdness axis; temperature, erosion, and depth feed the like-named axes.
- QuartPos.toBlock converts a quart coordinate q to a block coordinate as q << 2, with no offset, no centering, and no interpolation.
- Vanilla's climate sampler has no "fuzz" or jitter offset anywhere in its quart-to-block coordinate conversion.
- Climate.Sampler.sample(quartX, quartY, quartZ) evaluates its six climate density functions at the single exact block position (quartX<<2, quartY<<2, quartZ<<2), via a single-point, non-interpolated evaluation context.
- Vanilla narrows each climate density function's f64 output to f32 before quantizing it to the integer climate value; the narrowing path is always f64 -> f32 -> quantize_climate, never f64 fed directly into the quantizer's f32 parameter.
- Rust's "as f32" narrowing cast on a finite f64 (round-to-nearest, ties-to-even) and its "as i64" narrowing cast on a finite f32 (truncate toward zero) both match Java's own narrowing-cast semantics exactly for this climate-sampling path.
- In a climate target point, the target value for the offset axis is always implicitly 0.
- Vanilla's Climate.Parameter.distance(min, max, t) is: above = t - max, below = min - t, distance = above if above > 0 else max(below, 0) -- i.e. exactly 0 when t is within [min, max], and never negative.
- Java's Mth.square(long) computes x*x using wrapping (unchecked) multiplication on overflow.
- Quantized climate values are bounded to roughly +/-20000 per axis.
- A biome-parameter point's fitness against a target is the sum of squared per-axis distances (via Climate.Parameter.distance) over the six named climate axes (temperature, humidity, continentalness, erosion, depth, weirdness) plus a seventh synthetic "offset" axis, whose distance to the target equals the point's own offset value directly, since the target's offset is implicitly 0.
- Vanilla's real accelerated biome-search structure is not a classic balanced R-tree but a recursive k-d-tree-style bucketing structure over all 7 quantized axes, with a parameter count of 7 and 6 children per node.
- In vanilla's real search structure, each leaf holds one climate point, represented as seven axis spans (the six named axes' spans plus a seventh synthetic point-interval span for offset), paired with its biome-entry index.
- When building vanilla's real search structure, if a node has 6 or fewer children, they are sorted by their total absolute center-of-interval magnitude summed across all 7 axes (sum over axes of the absolute value of (span.min+span.max)/2, using integer division truncating toward zero) and wrapped directly into one internal node.
- When building vanilla's real search structure with more than 6 children (n = children.len()), the bucket size used is 6 raised to the power floor(log base 6 of (n - 0.01)).
- When building vanilla's real search structure with more than 6 children, for each of the 7 axes independently the children are sorted by that axis's own interval center, with ties broken by cycling through the remaining axes in rotation: (axis+1) mod 7, then (axis+2) mod 7, and so on through all 7 axes if needed.
- When building vanilla's real search structure with more than 6 children, the axis-sorted children are split into consecutive buckets of the computed bucket size, with the final bucket possibly shorter than the rest.
- Each bucket's cost in vanilla's real search structure build is the sum, over all 7 axes, of the absolute difference between that axis's bucket-wide bounding maximum and minimum, not squared, unlike the fitness/distance metric.
- When building vanilla's real search structure, the axis whose buckets have the lowest total summed cost is chosen as that node's split axis.
- When building vanilla's real search structure, the winning axis's bucket set is re-sorted lexicographically by the absolute center magnitude of each bucket's own bounding box, using the same seven-axis comparator chain as the per-axis trial sort (starting at the winning axis and rotating through the rest), not the single summed-magnitude key used for leaf-level fan-out, before the build recurses into each bucket.
- At an internal node of vanilla's real search structure, the search evaluates the same fitness-shaped distance metric against every child's own bounding 7-span, in the node's own stored child order.
- Vanilla's real search structure's branch-and-bound search recurses into a child only when that child's metric is strictly less than the current best-known fitness found so far.
- A leaf of vanilla's real search structure updates the running best-fitness candidate using the same strict-less-than (never less-than-or-equal) rule as a naive linear scan.
- Vanilla additionally seeds the very first "best" candidate of a real search from a thread-local cache of the last successfully found leaf (exploiting spatial coherence between consecutive queries); because the prune and best-update tests are both strict, a cached leaf whose distance equals the eventual minimum is never displaced and also blocks descent into any subtree whose bound equals that minimum, so the cache changes the final search result relative to a cold start whenever an exact fitness tie exists, and is result-neutral only on tie-free data.
- Child iteration order in vanilla's real search structure is parity-relevant whenever two children tie exactly on distance, since the first-encountered child wins the tie, and the real tree's search must preserve the exact children ordering established by its own build step to reproduce that.
- Adjacent Overworld climate bands are defined so that they share an exact edge value on at least one axis, for example a temperature value of -0.45.
- Vanilla's spawn-point search (Climate.findSpawnPosition) searches against NoiseGeneratorSettings' own spawn-target list, a separate, small list of climate points, never the full biome-parameter list.
- Vanilla's spawn-point search walks an Archimedean-spiral radial pattern outward from a center point.
- Vanilla's spawn-point search samples climate at a fixed quart-Y of 0, since the search is Y-agnostic and the depth axis handles vertical placement quality instead.
- Vanilla's spawn-point search ranks each candidate position by the formula (minimum fitness over all spawn targets) * 2048^2 + (block_x^2 + block_z^2), so climate fitness dominates the ranking unless two candidates are within 1/2048^2 "worth" of origin-distance of each other.
- Vanilla's spawn-point search forces the queried climate point's own depth axis to 0 before computing fitness against the spawn targets (zeroDepthTargetPoint).
- Vanilla's spawn-point search truncates (does not round) the candidate x/z block coordinates computed from sin(angle)*radius and cos(angle)*radius.
- The first pass of vanilla's spawn-point radial search initializes radius to 512 (not 0) and sweeps up to and including 2048 in increments of 512, visiting only 512, 1024, 1536 and 2048; the origin is scored exactly once before either pass, never re-scored inside one.
- The second pass of vanilla's spawn-point radial search is centered on the first pass's own best-found result and sweeps radius out to 512 in increments of 32.
- QuartPos.fromBlock converts a block coordinate to a quart coordinate using a floor-toward-negative-infinity convention, i.e. an arithmetic right-shift by 2 rather than a truncating division.
- Vanilla's world height spans from Y = -64 up through a total height of 384 blocks, both exact multiples of 4 with no rounding needed when converting to quart coordinates.
