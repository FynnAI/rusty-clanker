...
`sqrt`/`ln` must be plain IEEE-754 `f64::sqrt`/`f64::ln`. Of the two, only `ln` is the source of cross-platform uncertainty: vanilla's `Math.log` is HotSpot-JIT-intrinsified and carries a weak contract (within 1 ulp of the exact result, semi-monotonic), so it is not guaranteed bit-identical across JVM implementations/hardware; `Math.sqrt`, though also `@IntrinsicCandidate`-marked, is specified as the correctly rounded, IEEE-754 `squareRoot` operation and is therefore bit-identical on every conforming JVM and equal to Rust's `f64::sqrt`. This is nonetheless the one spot in this whole module where "should match vanilla on all real targets" is a documented, structurally weaker guarantee than "provably always matches" — the weaker guarantee still governs every Gaussian golden vector, because the Gaussian multiplier is `x * sqrt(-2*ln(s)/s)`, so a 1-ulp difference in `ln` propagates through the exactly-rounded `sqrt` into the final result. Flag this in the acceptance suite (tolerance-based, §Acceptance tests) rather than asserting exact equality for Gaussian-only tests, while every integer/long/hash/seed-derivation vector elsewhere in this blueprint remains an exact-equality assertion — a standalone `f64::sqrt` assertion outside a Gaussian computation may still be asserted exactly.
...
fn seed_slime_chunk(x: i32, z: i32, world_seed: i64, salt: i64) -> RcLegacyRandom:
    x_sq_term = sign_extend_i32(wrapping_mul_i32(wrapping_mul_i32(x, x), 4987142))   # i32 wrap, THEN sign-extend
    x_term    = sign_extend_i32(wrapping_mul_i32(x, 5947611))                        # i32 wrap, THEN sign-extend
    z_sq_term = wrapping_mul(sign_extend_i32(wrapping_mul_i32(z, z)), 4392871)       # inner z*z wraps at i32; only THIS product is genuinely i64 (the sole `L`-suffixed literal)
    z_term    = sign_extend_i32(wrapping_mul_i32(z, 389711))                         # i32 wrap, THEN sign-extend
    inner = wrapping_add(wrapping_add(wrapping_add(wrapping_add(world_seed, x_sq_term), x_term), z_sq_term), z_term) XOR salt
    return RcLegacyRandom::new(inner)
```

(Arithmetic widths matter as much as precedence here: only `z*z*4392871` is a genuinely 64-bit multiply — vanilla's Java source carries the sole `L` suffix in this whole expression on that one literal — so `x*x*4987142`, `x*5947611`, the inner `z*z`, and `z*389711` are each evaluated in 32-bit WRAPPING `int` arithmetic and only then sign-extended into the `i64` sum; computing any of them directly in `i64` space (the "obviously correct" naive translation) diverges from vanilla the moment that term's own 32-bit product would overflow — `x*x*4987142` at `|x| >= 21`, `x*5947611` at `|x| >= 362`, the inner `z*z` at `|z| >= 46341`, and `z*389711` at `|z| >= 5511`. Operator precedence matters too: XOR applies to the ENTIRE summed expression, not just its last term — vanilla's own Java source binds `^ salt` looser than every `+`/`*` term, so the pseudocode above computes the full sum first, then XORs once at the end, matching that precedence exactly rather than folding `salt` into one of the intermediate `wrapping_add` calls.) Vanilla's own call site uses `salt = 987234911` (a caller-supplied literal at that one call site, e.g. `Slime.java` — not baked into this function); the caller then draws exactly one `next_int_bounded(10) == 0` to answer "is this a slime chunk," which is out of this blueprint's own scope (a future M5/game-mechanics consumer's single line of code against this function's return value).

### J. Seed-string parsing (GEN-D2)

```text
fn parse_seed_string(input: &str) -> Option<i64>:
    trimmed = input.trim()
    if trimmed.is_empty():
        return None                                     # caller substitutes a freshly generated random seed
    match trimmed.parse::<i64>():
        Ok(v) => Some(v)
        Err(_) => Some(java_string_hash_code(trimmed) as i64)
```

Vanilla trims the input first, and treats an empty or whitespace-only trimmed result as "no seed given," never as seed `0` — the caller substitutes a freshly generated random seed in that case, exactly as `parse_seed_string` returning `None` requires its own caller to do here. Only once that empty check passes does vanilla attempt an integer parse of the trimmed string, falling back on failure to hashing the TRIMMED string (not the raw input). Rust's `i64::from_str` accepted grammar matches Java's `Long.parseLong`'s for ASCII input (optional leading `+`/`-`, decimal digits, no embedded whitespace, no underscore digit separators, `i64`-range-checked with overflow rejected rather than wrapped), but the two grammars are not fully byte-for-byte compatible: `Long.parseLong` also accepts any Basic-Multilingual-Plane Unicode decimal digit for which `Character.digit(char, 10)` is non-negative — general category Nd, e.g. Arabic-Indic U+0660–0669, Extended Arabic-Indic U+06F0–06F9, Devanagari U+0966–096F, Fullwidth U+FF10–FF19 (but not supplementary-plane Nd digits, since `Long.parseLong` iterates UTF-16 `char` values one at a time and a supplementary-plane digit arrives as an unpaired surrogate) — which Rust's parser rejects (ASCII `0`–`9` only). A seed string built from a non-ASCII BMP Unicode digit therefore takes the numeric path in vanilla and the hash path in a naive Rust port, producing a different world; this is a genuine, accepted incompatibility between the two grammars, not merely a hypothetical one.

This blueprint's own verification pass compiled and ran a battery of edge cases through Rust's actual `str::parse::<i64>()` and confirmed every one behaves identically to the documented Java grammar for ASCII input: `"5"`, `"+5"`, `"-5"`, `"007"` (leading zeros allowed, parses to `7`), and both `i64::MIN`/`i64::MAX` boundary strings all parse successfully; a lone `"+"` or `"-"`, one-past-`i64::MAX` (`"9223372036854775808"`, overflow), non-numeric text, and an underscore-separated numeral (`"1_000"`) all fail to parse in both languages and correctly fall through to the `java_string_hash_code` branch; embedded whitespace (`" 5"`, `"5 "`) is trimmed away before the parse attempt and so parses successfully as `5`, never reaching the hash branch; and the empty string (or any whitespace-only string) yields no seed at all.
...
- Vanilla's legacy RNG is a 48-bit LCG, bit-compatible with java.util.Random.
- Vanilla's legacy RNG holds one 48-bit-masked i64 of state; vanilla's Xoroshiro RNG holds two i64 state words.
- Legacy's nextInt(bound) uses a power-of-two fast path plus a next(31)-modulo rejection loop; Xoroshiro's nextInt(bound) uses Lemire multiply-high rejection with no power-of-two fast path.
- Vanilla's nether, end, caves, and floating_islands noise-settings presets set legacy_random_source: true (use the legacy LCG); overworld, large_biomes, and amplified presets set legacy_random_source: false (use Xoroshiro).
- Vanilla slime chunks always use the legacy RNG regardless of dimension.
- Vanilla's structure-spread jitter (RandomSpreadStructurePlacement) always uses the legacy RNG regardless of dimension.
- Vanilla's random_sequence (used for loot) always uses the Xoroshiro RNG regardless of dimension.
- Vanilla's RandomSource.nextIntBetweenInclusive(min, maxInclusive) is defined as nextInt(maxInclusive - min + 1) + min.
- Vanilla's RandomSource.nextIntRange(origin, boundExclusive) is defined as origin + nextInt(boundExclusive - origin).
- Vanilla's RandomSource triangle(mean, spread) helper is defined as mean + spread * (nextDouble() - nextDouble()), with the second draw subtracted.
- Vanilla's RandomSource consume-count helper draws exactly one nextInt() call per round, discarding the result, for the given number of rounds.
- Calling nextInt(bound) with bound <= 0 throws Java's IllegalArgumentException in vanilla.
- Legacy LCG multiplier constant is 25214903917 (0x5DEECE66D).
- Legacy LCG addend constant is 11 (0xB).
- Legacy LCG modulus mask is 281474976710655 (0xFFFFFFFFFFFF, i.e. 2^48 minus 1).
- Legacy setSeed computes state = (seed XOR 25214903917) AND 281474976710655, and resets the Gaussian cache.
- Legacy next(bits) computes state = (state * 25214903917 + 11) AND 281474976710655, then returns the top `bits` bits of the new state (an arithmetic right shift by 48-bits, harmless since the top 16 bits of state are always zero).
- Legacy nextInt() returns next(32), the top 32 bits of one LCG step.
- Xoroshiro nextInt() returns the low 32 bits (truncating cast) of a freshly drawn nextLong().
- Legacy's power-of-two fast path for nextInt(bound) computes ((bound as i64) * (next(31) as i64)) >> 31 using a single draw.
- Xoroshiro's nextInt(bound) never uses a power-of-two fast path; every bound, including powers of two, goes through the same multiply/threshold/reject Lemire logic.
- Legacy nextLong() is ((next(32) as i64) << 32) + (next(32) as i64) - two calls, high word drawn first, combined with addition (not bitwise or) because the low word is sign-extended and can be negative.
- Xoroshiro nextLong() is a single raw core step (next_u64 as i64), with no combination of two draws.
- Legacy nextBoolean() is next(1) != 0.
- Xoroshiro nextBoolean() is (nextLong() & 1) != 0.
- Legacy nextFloat() is (next(24) as f32) * FLOAT_UNIT.
- Xoroshiro nextFloat() takes the top 24 of 64 bits of one nextLong() (logical shift right by 64-24), then multiplies by FLOAT_UNIT.
- Legacy nextDouble() is (((next(26) as i64) << 27) + (next(27) as i64)) as f64 * DOUBLE_UNIT, from two next(bits) calls split 26-then-27.
- Xoroshiro nextDouble() takes the top 53 of 64 bits of one nextLong() (logical shift right by 64-53), then multiplies by DOUBLE_UNIT.
- Legacy's nextInt(bound) rejection loop draws bits = next(31), computes val = bits % bound (Java's % on a non-negative i32 operand is a plain remainder), and accepts val when the 32-bit wrapping computation (bits - val + bound - 1) is >= 0, otherwise redraws.
- Legacy's rejection-loop acceptance test must be computed in wrapping 32-bit arithmetic; computing it in a wider integer type changes the rejection rate (and therefore call count) for a fraction of (bits, bound) pairs near i32::MAX.
- Xoroshiro's Lemire nextInt(bound) algorithm draws random_bits as the zero-extended low 32 bits of a fresh nextLong(), computes product = random_bits * bound as u64 (exact, both operands under 2^32), takes frac = product AND 0xFFFFFFFF, and if frac is less than bound computes threshold as Java's Integer.remainderUnsigned(-bound, bound) (an unsigned 32-bit remainder) and redraws while frac stays below threshold; the result is (product >> 32) as i32.
- Vanilla's Integer.remainderUnsigned(-bound, bound) threshold in the Lemire algorithm must be computed as a genuinely unsigned 32-bit remainder; a signed remainder gives the wrong threshold for roughly half of all bound values, because -bound as a signed i32 is negative while the unsigned reinterpretation is 2^32 - bound.
- Vanilla's FLOAT_UNIT and DOUBLE_UNIT constants are used unmodified by both RNG families' nextFloat/nextDouble implementations (Legacy's BitRandomSource.FLOAT_MULTIPLIER/DOUBLE_MULTIPLIER and Xoroshiro's own next_float/next_double reuse the identical two constants).
- Vanilla's nextGaussian() uses the Marsaglia polar method with a per-instance single-value cache that setSeed resets.
- Vanilla's nextGaussian(): if a cached value exists, consume and return it; otherwise loop drawing v1 = 2*nextDouble()-1 and v2 = 2*nextDouble()-1 until s = v1*v1+v2*v2 is less than 1.0 and not equal to 0.0, then set mul = sqrt(-2*ln(s)/s), cache v2*mul, and return v1*mul.
- Vanilla's nextGaussian() rejection loop draws two nextDouble() calls per iteration, with an expected approximately 1.273 iterations per fresh pair (acceptance probability pi/4, approximately 78.5%).
- Every nextGaussian() call other than the one that triggers a fresh pair draw is "free" - it consumes zero RNG draws and returns the previously cached value.
- A fresh Gaussian pair costs four raw LCG steps under the legacy RNG (two next(bits) calls inside nextDouble(), called twice) but only two raw state transitions under Xoroshiro (one nextLong() inside nextDouble(), called twice).
- Vanilla's Math.log used inside nextGaussian() is HotSpot-JIT-intrinsified and carries only a within-1-ulp, semi-monotonic contract, so it is not guaranteed bit-identical across different JVM implementations or hardware; Math.sqrt, also intrinsified, is specified as the correctly rounded IEEE-754 squareRoot operation and is bit-identical on every conforming JVM, equal to Rust's f64::sqrt, though the Gaussian multiplier sqrt(-2*ln(s)/s) still inherits ln's cross-platform uncertainty through that otherwise-exact sqrt.
- Xoroshiro's golden-ratio-64 mixing constant is -7046029254386353131 (0x9E3779B97F4A7C15).
- Xoroshiro's silver-ratio-64 mixing constant is 7640891576956012809 (0x6A09E667F3BCC909).
- Xoroshiro's stafford-13 mix first multiplier constant is -4658895280553007687 (0xBF58476D1CE4E5B9).
- Xoroshiro's stafford-13 mix second multiplier constant is -7723592293110705685 (0x94D049BB133111EB).
- Vanilla's stafford-13 mix function computes z = (z XOR (z logical-shift-right 30)) * stafford_mul_1, then z = (z XOR (z logical-shift-right 27)) * stafford_mul_2, then returns z XOR (z logical-shift-right 31), with every multiply/add performed as 64-bit wrapping arithmetic and all three shifts unsigned (logical).
- Vanilla's 128-bit seed upgrade (unmixed) computes lo = seed XOR silver_ratio_64 and hi = lo + golden_ratio_64 (wrapping add).
- Vanilla's full 128-bit seed upgrade applies the stafford-13 mix to both the lo and hi halves produced by the unmixed upgrade.
- A genuinely fresh top-level Xoroshiro construction from one i64 seed applies the full mixed seed upgrade (stafford-13 mix on both halves).
- Vanilla's create_random_sequence applies its own separate, explicit stafford-13 mixing step at the very end of its own derivation, after any optional XORs.
- Vanilla's Xoroshiro positional-factory output and its fork()/fork_positional() output are used directly as a raw state pair, with no stafford-13 mixing applied (aside from the all-zero-state guard).
- Vanilla's xoroshiro128++ core step uses output rotation 17, s0 rotation 49, an XOR-shift of 21, and s1 rotation 28.
- Vanilla's xoroshiro128++ nextLong() computes result = rotate_left(s0+s1, 17) + s0 (wrapping), then updates s1 ^= s0 and sets state to (rotate_left(s0,49) XOR s1 XOR (s1 << 21), rotate_left(s1,28)), returning the original result.
- Vanilla's own all-zero-seed substitution for a 128-bit Xoroshiro state pair is (golden_ratio_64, silver_ratio_64), not the SplitMix64-derived fallback that the rand_xoshiro crate's own from_seed applies to an all-zero 16-byte seed.
- Vanilla's fork() draws two nextLong() calls (not re-mixed) to produce a new raw state pair; fork_positional() likewise draws two nextLong() calls (not re-mixed) to produce a new positional-factory seed pair.
- Vanilla's positional seed hash mth_get_seed(x,y,z) computes x_term as x*3129871 performed in 32-bit space and only then sign-extended to i64.
- Vanilla's positional seed hash computes z_term as z (sign-extended to i64) multiplied by 116129781 genuinely in 64-bit space (the Java source literal carries an L suffix).
- Vanilla's positional seed hash combines seed = x_term XOR z_term XOR (y sign-extended to i64), then computes seed = seed*seed*42317861 + seed*11 (wrapping), and returns seed arithmetic-shifted right by 16 bits (a signed, sign-preserving shift).
- Computing x*3129871 directly in i64 space instead of 32-bit-then-sign-extend diverges from vanilla for any x whose product with 3129871 overflows i32, which occurs routinely for absolute values of x past roughly 686.
- Vanilla's Java String.hashCode(), used by java_string_hash_code, iterates the string's UTF-16 code units (including surrogate-pair halves for non-BMP characters), computing h = h*31 + unit (wrapping) starting from h=0.
- Vanilla's java_string_hash_code("hello") equals 99162322, and java_string_hash_code("") equals 0.
- Vanilla's md5_seed(name) computes the MD5 digest of name's UTF-8 bytes and splits it into two big-endian i64 halves: lo from the first 8 digest bytes, hi from the last 8 digest bytes.
- The Xoroshiro positional factory's from_hash_of uses the raw (unmixed) md5_seed halves directly, with no stafford-13 mixing applied at that call site.
- Vanilla's legacy positional factory computes .at(x,y,z) as a legacy RNG seeded with mth_get_seed(x,y,z) XOR the factory's own seed.
- Vanilla's Xoroshiro positional factory computes .at(x,y,z) by perturbing only the low seed half with mth_get_seed(x,y,z) XOR seed_lo, while the high seed half (seed_hi) passes through unchanged.
- Vanilla's legacy positional factory computes .from_hash_of(name) as a legacy RNG seeded with (java_string_hash_code(name) as i64) XOR the factory's own seed.
- Vanilla's Xoroshiro positional factory computes .from_hash_of(name) by XORing each md5_seed half of name with the corresponding seed_lo/seed_hi half.
- Vanilla's legacy positional factory's .from_seed(s) constructs a legacy RNG directly from s, ignoring the factory's own stored seed field entirely.
- Vanilla's Xoroshiro positional factory's .from_seed(s) constructs a raw state pair (s XOR seed_lo, s XOR seed_hi).
- Vanilla's WorldgenRandom wrapper's own nextInt/nextInt(bound)/nextLong/nextFloat/nextDouble are always computed via the legacy-style formulas built exclusively on the wrapped source's next(bits), even when the wrapped source is the Xoroshiro RNG.
- Vanilla's WorldgenRandom class extends LegacyRandomSource and overrides only its next(bits) primitive to forward to whichever wrapped RNG source it carries; every other derived-value method is inherited unmodified from the legacy family's own implementation, itself built exclusively on next(bits).
- Calling next_int_bounded through a WorldgenRandom wrapping the Xoroshiro RNG runs the legacy rejection-loop algorithm applied to Xoroshiro-sourced bits, never Xoroshiro's own native Lemire algorithm; the Lemire algorithm only runs when next_int_bounded is called directly on a bare Xoroshiro RNG instance (e.g. via create_random_sequence).
- For seed 0 and bound 10, a WorldgenRandom wrapping the Xoroshiro RNG yields the four next_int_bounded(10) values [8, 5, 8, 6], while the bare Xoroshiro RNG's own native Lemire next_int_bounded(10) for the same seed yields [9, 1, 1, 3].
- For seed 0 and bound 10, a WorldgenRandom wrapping the legacy RNG is bit-identical to a bare legacy RNG, both yielding the values [0, 8, 9, 7, 5].
- Vanilla's WorldgenRandom.setSeed override forwards only to the wrapped inner RNG's own setSeed (resetting that inner source's private Gaussian cache); it does not reset WorldgenRandom's own separate Gaussian cache, inherited unmodified from LegacyRandomSource.
- Vanilla's set_decoration_seed(world_seed, chunk_min_x, chunk_min_z) sets the seed to world_seed, draws x_scale = nextLong()|1 and z_scale = nextLong()|1 (both forced odd), computes result = (chunk_min_x*x_scale + chunk_min_z*z_scale) XOR world_seed (wrapping), reseeds with result, and returns result.
- Vanilla's decoration/population seed method was historically nicknamed setPopulationSeed before version 1.18; vanilla's current source names it setDecorationSeed.
- Vanilla's set_feature_seed(decoration_seed, index, step) reseeds with decoration_seed + index + step*10000 (wrapping), a pure arithmetic operation that consumes zero RNG draws.
- In vanilla, set_feature_seed's index parameter is a global index assigned once across every biome by FeatureSorter's DFS-topological sort over the whole biome registry's iteration order, not a per-biome-list-local counter; its step parameter is the decoration-step ordinal, ranging 0 to 10 across vanilla's 11 fixed decoration steps.
- Vanilla's set_large_feature_seed(seed, chunk_x, chunk_z) reseeds with seed, draws x_scale = nextLong() and z_scale = nextLong() used unmodified (not forced odd, unlike decoration seed), computes result = (chunk_x*x_scale) XOR (chunk_z*z_scale) XOR seed, and reseeds with result.
- Vanilla calls set_large_feature_seed once per (source_chunk, carver) pair as set_large_feature_seed(world_seed + carver_index, source_chunk_x, source_chunk_z), folding the carver's own additive offset into the seed parameter itself before the call rather than applying it separately afterward.
- Vanilla's carver_index is scoped per source chunk, reset to 0 for every one of the 289 neighbor chunks visited by the 17x17 source-chunk neighborhood scan, and is never a global index shared with set_feature_seed's own FeatureSorter indexing.
- Vanilla's set_large_feature_with_salt(seed, x, z, salt) reseeds with (x*341873128712 + z*132897987541) + (seed + salt), a pure arithmetic operation consuming zero RNG draws.
...
- Every unmodified vanilla 26.2 world uses salt = 0, include_world_seed = true, and include_sequence_id = true as its create_random_sequence per-world defaults.
- Vanilla's slime-chunk seeding (seed_slime_chunk) always constructs a fresh, one-shot legacy RNG, deliberately kept on the classic LCG for backward compatibility with pre-1.18 "slime seed" community tooling, independent of whichever RNG algorithm the hosting dimension's own terrain uses.
- Vanilla's seed_slime_chunk(x, z, world_seed, salt) computes inner = world_seed + (x*x*4987142, wrapped in 32-bit int space then sign-extended) + (x*5947611, wrapped in 32-bit int space then sign-extended) + ((z*z, wrapped in 32-bit int space then sign-extended) * 4392871, genuinely 64-bit since only that literal carries an L) + (z*389711, wrapped in 32-bit int space then sign-extended), all summed and only then XORed once with salt over the entire expression (matching Java operator precedence, where ^ binds looser than + and *).
- Vanilla's own slime-chunk call site uses salt = 987234911 (e.g. in Slime.java), then draws exactly one next_int_bounded(10) and treats a result of 0 as "this is a slime chunk."
- Vanilla's seed-string parsing (parse_seed_string) trims the input first, yields no seed at all (letting its caller substitute a freshly generated random seed) when the trimmed result is empty or whitespace-only, and only then tries to parse the trimmed string as an i64, falling back on failure to treating java_string_hash_code of the TRIMMED string (cast to i64) as the seed.
- Java's Long.parseLong grammar accepts an optional leading + or -, any Basic-Multilingual-Plane Unicode decimal digit for which Character.digit(char, 10) is non-negative (not ASCII decimal digits only — supplementary-plane decimal digits are rejected, since parsing iterates UTF-16 char values one at a time), no embedded whitespace, no underscore digit separators, and range-checks to i64 with overflow rejected rather than wrapped.
- Vanilla seed-string parsing test cases confirmed to parse successfully as integers: "5", "+5", "-5", "007" (leading zeros allowed, parses to 7), and both the i64::MIN and i64::MAX boundary strings.
- Vanilla seed-string parsing test cases confirmed to fail integer parsing and fall through to the string-hash branch (hashing the TRIMMED string): a lone "+" or "-", one past i64::MAX ("9223372036854775808"), non-numeric text, and an underscore-separated numeral ("1_000"); strings with surrounding whitespace (" 5", "5 ") are trimmed first and parse successfully as 5, never reaching the hash branch, and the empty string (or any whitespace-only string) yields no seed at all, with the caller substituting a freshly generated random seed rather than falling through to a hash.
- Vanilla's FLOAT_UNIT constant equals 2^-24 exactly, with bit pattern 0x33800000.
- Vanilla's DOUBLE_UNIT constant equals 2^-53 exactly, with bit pattern 0x3CA0000000000000.
...
```rust
/// GEN-D2 world-seed-string grammar. Trims `input` first; an empty or
/// whitespace-only trimmed result yields `None` (no seed given — the caller
/// substitutes a freshly generated random seed, matching vanilla's
/// `WorldOptions.parseSeed`/`randomSeed` split). Otherwise, a trimmed string
/// that parses as a Java `long` (Rust's `i64::from_str` — grammar-compatible
/// for ASCII input, though not fully byte-for-byte: `Long.parseLong` also
/// accepts non-ASCII BMP Unicode decimal digits Rust's parser rejects) is
/// used directly; on parse failure, the seed is `hash::java_string_hash_code`
/// of the TRIMMED string, sign-extended.
pub fn parse_seed_string(input: &str) -> Option<i64>;
```
...
9. `random_zero_next_gaussian_matches_published_vector` — `RcLegacyRandom::new(0)`, two `next_gaussian()` calls; assert `1e-9`-tolerance equality (Context §E's documented `ln` cross-platform caveat, which propagates through the otherwise-exact `sqrt`) to `[0.8025330637390305, -0.9015460884175122]` (published).
...
10. `xoroshiro_gaussian_two_calls_moderate_tolerance` — `RcXoroshiroRandom::new(0)`, two `next_gaussian()` calls; assert `1e-9`-tolerance equality to `[-0.48540690699780015, 0.43399227545320296]` (blueprint-derived, same `ln` cross-platform caveat, propagating through the otherwise-exact `sqrt`, as the legacy Gaussian test — self-derived by this blueprint's own pass, not independently cross-checked against a live JVM).
...
14. `seed_slime_chunk_large_coordinates_regression_guard_against_i64_space_overflow` — `seed_slime_chunk(1_000_000, 100_000, world_seed=0, salt=987234911)` (both coordinates deliberately past every one of Context §I's four int-wrap overflow thresholds — `|x| >= 21`, `|x| >= 362`, `|z| >= 46341`, `|z| >= 5511` — none of the other vectors in this test file exercise any of them, since they all stay at `{0,0}`/`{5,-3}`), one `next_int_bounded(10)`; assert `8` (blueprint-verified) — and assert this DIFFERS from the value a naive all-i64-space translation of the same formula would produce for the same inputs (hand-computed in the test itself, `4`) — a concrete regression guard against exactly the "computed every term in i64 space instead of wrapping each at i32 first" mistake Context §I flags as this formula's own highest-risk hazard.
15. `worldgen_random_gaussian_cache_survives_set_seed_reseed` — construct TWO separate `LegacyWorldgenRandom::new(RcLegacyRandom::new(0))` instances, `a` and `b`. On `b` (the control): call `next_gaussian()` twice with no reseed in between; record `b`'s SECOND result — by the algorithm's own construction (Context §E), this is exactly the cached value the first call computed and stashed. On `a` (the case under test): call `next_gaussian()` once (populates `a`'s own wrapper-level cache identically to `b`'s first call, since both start from the same seed), then call `set_decoration_seed(1, 0, 0)` on `a` (reseeds `a`'s wrapped `inner` only, per `WorldgenRandom`'s `BitSource` impl), then call `next_gaussian()` on `a` once more. Assert `a`'s post-reseed result equals `b`'s recorded second value — proving the reseed did NOT clear `a`'s wrapper-level cache (the leak Context §H documents genuinely reproduces), without requiring any internal-state inspection. This is a mechanical, isolated-unit proof of the documented quirk, explicitly NOT claiming any real vanilla call site exercises this path (Context §H's own stated open question).

### `crates/rng/tests/seed_string_parsing.rs` (pure)

1. `numeric_seed_strings_parse_directly` — `parse_seed_string("12345") == Some(12345)`, `parse_seed_string("-12345") == Some(-12345)`, `parse_seed_string("+42") == Some(42)`, `parse_seed_string("007") == Some(7)`, `parse_seed_string("9223372036854775807") == Some(i64::MAX)`, `parse_seed_string("-9223372036854775808") == Some(i64::MIN)` (all blueprint-verified via a compiled Rust program during this blueprint's own derivation pass).
2. `non_numeric_seed_strings_fall_back_to_string_hash_code` — `parse_seed_string("hello world") == Some(1794106052)`, `parse_seed_string("Rusty Clanker") == Some(616675319)` (blueprint-derived) — each hashes the TRIMMED input, unchanged here since neither string carries leading/trailing whitespace.
3. `overflow_and_malformed_numeric_strings_fall_back_to_hash_not_saturate` — `parse_seed_string("9223372036854775808")` (one past `i64::MAX`) equals `Some(java_string_hash_code("9223372036854775808") as i64)`, NOT a saturated/clamped `i64::MAX` — proves overflow is treated as a parse failure (falls through to hashing), matching Java's `NumberFormatException`-on-overflow behavior, not silently clamped.
4. `whitespace_is_trimmed_before_the_numeric_path_runs` — `parse_seed_string(" 5") == Some(5)` and `parse_seed_string("5 ") == Some(5)` (both trim to `"5"` and parse successfully, never reaching the hash branch); `parse_seed_string("1_000")` equals `Some(java_string_hash_code("1_000") as i64)`, since neither Java's `Long.parseLong` nor Rust's `i64::from_str` has an underscore-digit-separator grammar.
5. `empty_or_whitespace_only_seed_string_yields_no_seed` — `parse_seed_string("")`, `parse_seed_string("   ")`, and `parse_seed_string("\t\n")` (a whitespace-only string trims to empty) all return `None` — proving the caller must substitute a freshly generated random seed rather than treating an absent seed string as seed `0`.
6. `padded_non_numeric_seed_string_hashes_the_trimmed_string_not_the_padded_one` — `parse_seed_string(" abc ")` equals `Some(java_string_hash_code("abc") as i64)`, i.e. `Some(96354)` (blueprint-verified) — NOT `java_string_hash_code(" abc ")` (`32539678`, a different value) — proving the trim happens before the hash fallback runs, not only before the numeric-parse attempt.
