...
Two further solidity predicates, distinct from the shape-occlusion fast paths above and reused by other Context sections: `is_solid(pos)` mirrors vanilla's own `isSolid()` cache (`calculateSolid`) — a `force_solid_on`/`force_solid_off` shape-property override takes precedence; failing that, the collision shape must be non-empty and either its bounding box's mean edge length is at least 0.7291666666666666 or its Y-extent is at least 1.0 (a bottom slab counts as solid despite not being a full cube — used by §C's source-conversion floor check). `blocks_motion(pos)` is `is_solid(pos)` further excluding cobweb and bamboo sapling by block kind (used by §H's `getFlow` redirect gate). Neither cobweb nor bamboo sapling exists in the current tier-1/tier-2 placeable set (Prerequisites), so `blocks_motion` reduces to `is_solid` for every block this blueprint's own acceptance tests can construct today — the kind-exclusion is kept only for definitional accuracy.

`is_solid_face(kind, pos, dir)` (used only by `getFlow`'s falling-redirect, §H, always called with a horizontal `dir` — the `direction == Up` branch vanilla defines is therefore dead in every call this blueprint makes) is, in real vanilla: false when the fluid at `pos` is `kind` (a same-fluid short-circuit); else false when `pos` is ice; else a genuine per-face sturdiness test (`SupportType.FULL` against the block's own *support* shape, not its collision shape) — not a whole-block full-cube test. This blueprint implements the same-fluid short-circuit and the ice exception exactly (via a caller-supplied `solid_face_exceptions` list, empty by default, reserved for ice's real content), but approximates the per-face sturdiness test with `is_full_cube(pos)` — the same documented, bounded simplification as this section's shape-occlusion fast paths above (the general per-face geometry test is not implemented; a bottom slab or similarly partial-shaped block therefore reads as non-solid-face here even where real vanilla's sturdiness test would call it solid), flagged for the same future blueprint that closes the `mergedFaceOccludes` gap.
...
- `is_solid_face` in real vanilla is false when the fluid at the position is the caller's own fluid kind (a same-fluid short-circuit), else false for ice, else a genuine per-face sturdiness test — not a full-cube test; this blueprint approximates that sturdiness test with `is_full_cube(pos)` as a documented, bounded simplification, the same scope-narrowing already declared for the general shape-occlusion case.
...
/// Context §F: vanilla's own solidity cache (`calculateSolid`) — a shape-property force-solid
/// override, else collision-shape non-empty AND (bounding-box mean edge length >= 0.7291666666666666
/// OR Y-extent >= 1.0). Used by §C's source-conversion floor check.
pub fn is_solid(world: &dyn BlockWorldAccess, pos: BlockPos) -> bool;
/// Context §F: `is_solid(pos)` further excluding cobweb and bamboo sapling by block kind. Used by
/// §H's `getFlow` redirect gate.
pub fn blocks_motion(world: &dyn BlockWorldAccess, pos: BlockPos) -> bool;
/// Context §F: false if the fluid at `pos` is `kind` (same-fluid short-circuit); false if `pos`
/// is in `tables.solid_face_exceptions` (ice reservation); else `is_full_cube(pos)` as a
/// documented, bounded approximation of vanilla's real per-face sturdiness test (the general
/// per-face geometry case is not implemented, mirroring this module's own shape-occlusion
/// simplification above). `dir` is accepted for signature-fidelity with vanilla but is never
/// `Up` in any call this blueprint makes and does not otherwise affect the result.
...
/// (`LiquidBlockContainer` at target ⇒ delegate to `can_place_liquid`, else `true`), further
/// gated by `!deny_hold_fluid.contains(target id)` — Context §J. Note this merges vanilla's own
/// split: real `canHoldSpecificFluid` carries no denylist at all — vanilla's denylist lives in
/// the separate `canHoldAnyFluid`, combined with the specific-fluid check only inside
/// `canHoldFluid` — this blueprint folds the two into one function as a deliberate
/// simplification.
