...
if sources >= 2 and gamerules.allows_source_conversion(this_kind):
    ...
    if is_solid(below) or (fluid_at(below).kind == this_kind and fluid_at(below).is_source()):
        return Source
...
Two gamerule-gated defaults, applied **per fluid, independently**: `water_source_conversion` defaults **true**; `lava_source_conversion` defaults **false** — two adjacent lava sources do **not** spontaneously create a third by default (ranked hazard #6). The source-conversion floor check is a genuine `OR`: a solid floor *or* an already-source cell directly below, either alone qualifies. `is_solid(pos)` here is vanilla's own solidity heuristic (`BlockBehaviour.BlockStateBase.isSolid`, cached from `calculateSolid`): a `force_solid_on`/`force_solid_off` shape-property override takes precedence, else the collision shape must be non-empty and either its bounding box's mean edge length is at least 0.7291666666666666 or its Y-extent is at least 1.0 — **not** a full-cube test, so a bottom slab (mean edge length ≈0.833) qualifies as a source-conversion floor despite not being a full cube.
...
`can_pass_through_wall`/`can_maybe_pass_through`/`can_hold_any_fluid`/`can_hold_specific_fluid` are vanilla's shape-occlusion gate (§3.4). This blueprint implements the fast paths the research corpus itself names: **target** shape == full cube ⇒ false; **source** shape == full cube ⇒ false, independently of the target (checked separately, and first); and **source** shape empty **and target** shape empty ⇒ true — the empty fast path requires **both** sides empty, so an empty target behind a full-cube source is rejected by the source-side check before the empty fast path is ever reached — via `rc-physics`'s `tier1_shape_table()`/`VoxelShape::full_cube()`/`VoxelShape::is_empty()`, **and treats every other, partial (non-full, non-empty) shape pairing as passable** — the general `Shapes.mergedFaceOccludes` per-face geometry merge is **not** implemented. This is a deliberate, bounded scope-narrowing, not an oversight: the general case is not among the research corpus's own ranked reimplementation hazards, and the current tier-1/tier-2 world content with any non-full shape (wire, torch, repeater, comparator — all thin, flat, or post-shaped) has no acceptance test in this blueprint requiring exact partial-face occlusion. Flagged for whichever future blueprint first ships real waterloggable content (stairs/slabs/fences) needing it. A caller-supplied `deny_hold_fluid` range list (empty by default, mirroring vanilla's own fixed denylist — doors, signs, ladder, sugar cane, bubble column, nether/end portal, end gateway, structure void — none of which exist as placeable content yet) additionally gates `can_hold_any_fluid` regardless of shape. `can_hold_any_fluid` and `can_hold_specific_fluid` are delivered as two separate functions matching vanilla's own real split exactly (Deliverables, `occlusion.rs`): `can_hold_any_fluid` is unconditionally true for a registered `WaterloggableRegistry` target, else false whenever `blocks_motion` holds, else gated by `deny_hold_fluid`; `can_hold_specific_fluid` carries no denylist of its own at all — a registered target delegates to `can_place_liquid`, every other target is unconditionally true — and the two are combined (`can_hold_any_fluid(target) && can_hold_specific_fluid(target)`) only inside `is_hole`, exactly mirroring vanilla's own `canHoldFluid` combinator.

Two further solidity predicates, distinct from the shape-occlusion fast paths above and reused by other Context sections: `is_solid(pos)` mirrors vanilla's own `isSolid()` cache (`calculateSolid`) — a `force_solid_on`/`force_solid_off` shape-property override takes precedence; failing that, the collision shape must be non-empty and either its bounding box's mean edge length is at least 0.7291666666666666 or its Y-extent is at least 1.0 (a bottom slab counts as solid despite not being a full cube — used by §C's source-conversion floor check). The override is a real, first-checked mechanism (Deliverables' `FluidTables.force_solid_on`/`force_solid_off`, two caller-supplied range lists, empty by default) rather than a design placeholder: verified against the reference, real vanilla's `forceSolidOn` applies only to cobweb, the in-flight `moving_piston` block, and every sign/hanging-sign variant, and `forceSolidOff` only to ladder — none of which exist in this blueprint's own tier-1/tier-2 placeable set (Prerequisites), so both lists stay empty for every block this blueprint's own acceptance tests can construct today, exactly the same "mechanism now, content later" shape as `deny_hold_fluid`/`solid_face_exceptions` above. The geometry half of the test is its own pure, directly-testable function, `is_solid_shape(shape: &VoxelShape) -> bool` (Deliverables) — empty shape ⇒ false, else the mean-edge-length/Y-extent check over the shape's own union bounding box (computed locally from `VoxelShape::boxes()`'s already-public `Aabb`/`Vec3` fields; `rc-physics` itself gains no new `bounds()` method) — with `is_solid(pos)` a thin wrapper that checks the override lists first and otherwise resolves the shape via `tier1_shape_table()` before delegating to it. `blocks_motion(pos)` is `is_solid(pos)` further excluding cobweb and bamboo sapling by block kind (used by §H's `getFlow` redirect gate). Neither cobweb nor bamboo sapling exists in the current tier-1/tier-2 placeable set, so `blocks_motion` reduces to `is_solid` for every block this blueprint's own acceptance tests can construct today — the kind-exclusion is kept only for definitional accuracy.

`is_solid_face(kind, pos, dir)` (used only by `getFlow`'s falling-redirect, §H, always called with a horizontal `dir` — the `direction == Up` branch vanilla defines is therefore dead in every call this blueprint makes) is, in real vanilla: false when the fluid at `pos` is `kind` (a same-fluid short-circuit); else false when `pos` is ice; else a genuine per-face sturdiness test (`SupportType.FULL` against the block's own *support* shape, not its collision shape) — not a whole-block full-cube test. This blueprint implements the same-fluid short-circuit and the ice exception exactly (via a caller-supplied `solid_face_exceptions` list, empty by default, reserved for ice's real content), and now implements the sturdiness test itself for real rather than approximating it: real vanilla's *support* shape defaults to the block's own collision shape unless a block overrides `getBlockSupportShape`, and verification against the reference confirms no block in this blueprint's own tier-1/tier-2 placeable set overrides it — so support shape and collision shape are the same `rc-physics` `VoxelShape` this blueprint already resolves via `tier1_shape_table()` for every other occlusion check, with no separate support-shape data of its own needed. The sturdiness test proper is its own pure, directly-testable function, `is_face_sturdy_shape(shape: &VoxelShape, dir: Direction) -> bool` (Deliverables): a face is sturdy iff some box in the shape fully spans `[0,1]` on both axes orthogonal to `dir` **and** touches the block boundary on `dir`'s own axis — exact, not an approximation, for every shape this blueprint's own tier-1/tier-2 table produces (a face is either fully covered by one box or not covered at all in every one of those shapes, verified by hand in the reference table below; never completed by merging two partial boxes). Only the fully general multi-box face-union merge (`Shapes.mergedFaceOccludes`'s own per-face geometry, needed only if some future block's face is completed by combining several partial boxes) stays unimplemented — the same class of bounded simplification as this section's shape-occlusion fast paths above, flagged for the same future blueprint.

**Tier-1/tier-2 shape reference**, hand-verified against `rc-physics`'s own hand-authored shapes (`crates/physics/src/shapes.rs`'s `build_tier1_table`), so `is_solid`/`is_solid_face` need no separate shape data of their own:

| Block shape | Extent X x Y x Z | Mean edge length | `is_solid` | Horizontal face sturdy |
|---|---|---|---|---|
| Ordinary terrain, furnace family (furnace, blast furnace, smoker), piston/sticky piston (unextended) — full cube | 1.0 x 1.0 x 1.0 | 1.0 | true | true |
| Hopper (top-rim box union funnel box) | 1.0 x 0.75 x 1.0 (union bounding box) | 0.9166666... | true | false (no single box covers a horizontal face) |
| Chest | 0.875 x 0.875 x 0.875 | 0.875 | true | false |
| Repeater or comparator | 1.0 x 0.125 x 1.0 | 0.7083333... | false | false |
| Redstone wire | 1.0 x 0.0625 x 1.0 | 0.6875 | false | false |
| Redstone torch (floor or wall) | 0.375 x 0.625 x 0.375 | 0.4583333... | false | false |

Hopper (and, less dramatically, chest) is this table's own load-bearing proof that the two predicates are not redundant restatements of each other: `is_solid` looks only at the shape's overall union bounding box (hopper's own mean edge, 0.9166..., clears the 0.7291666666666666 threshold, so hopper counts solid for §C's source-conversion floor check) while `is_solid_face` looks at each horizontal face on its own (neither the rim box nor the funnel box alone spans a full `[0,1]x[0,1]` face at any horizontal boundary, so no horizontal face is sturdy) — a reimplementation that tries to derive one predicate from the other gets hopper wrong in one direction or the other.
...
for dir in [North, East, South, West]:
    ...
    if neighbor_height == 0.0:
        if not blocks_motion(dir.apply(pos)):
...
if state.falling():
    for dir in [North, East, South, West]:
        if is_solid_face(this_kind, dir.apply(pos), dir) or is_solid_face(this_kind, Up.apply(dir.apply(pos)), dir):
...
**(A) Contact conversion — synchronous, neighbor-notification-driven, lava-only.** Runs from two trigger points on a **lava** cell: being placed, or a neighbor changing — this blueprint's `on_neighbor_changed` hook, plus an explicit call from `spread_to` immediately after it places a *new* lava cell (mirroring vanilla's own `onPlace` trigger; this crate's `BlockBehavior` trait has no separate placement hook, so `spread_to` is the one place a new fluid cell is actually written). A shape update does **not** trigger this reaction — vanilla's own `LiquidBlock.updateShape` never runs the contact-conversion scan, it only re-arms a fluid tick (§K); this blueprint's own `on_shape_update` hook mirrors that exactly: re-arm only, never a contact-conversion call. Scans **5 positions in fixed order `Up, North, South, West, East`** (a *different* order from §B's horizontal-only order) in a **single interleaved pass**, not two separate scans: at each position in turn, first test water ⇒ convert **this lava cell's own position** to **obsidian** if the lava is a source, else **cobblestone**, return immediately; only if that test does not fire, and the block directly below the lava is (caller-configured) soul soil, test the **same** position for (caller-configured) blue ice ⇒ convert to **basalt**, return immediately — both tests run against each position before the scan advances to the next, so a blue-ice match at an earlier position beats a water match at a later one. If no position matched either test across the whole scan, the lava is left alone (its own scheduled tick proceeds normally). This reaction is **not** gated by lava's own slow tick delay — it fires the same game tick the triggering neighbor-state-change is observed. Basalt conversion is optional (`ReactionBlocks::basalt_conversion: Option<...>`) — the primary, mandatory case is obsidian/cobblestone.
...
`spread_to(target_pos, from_direction, candidate)`: **(1)** if `this_kind == Lava` and `from_direction == Down` and the target's existing fluid is water: reaction B (§I) — convert target to stone, return, no lava placed. **(2)** else if the target is registered in `WaterloggableRegistry` and `can_place_liquid` accepts: write the waterlogged state via `ctx.set_block`, then self-arm a water fluid tick on behalf of the waterlog container (§J's `SimpleWaterlogged`/`WaterloggableBehavior` — in real vanilla this re-arm belongs to the container's own `placeLiquid` implementation, not to `FlowingFluid.spreadTo`, which schedules nothing at all; this crate's own `WaterloggableBehavior` trait is a pure state query with no `ctx` access, so `spread_to` performs the scheduling call here on the container's behalf — a structural design choice fixed by this crate's own trait boundary, kept here deliberately rather than moved, since `WaterloggableBehavior` has and needs no `ctx` access), return. **(3)** else: hard-overwrite via `ctx.set_block(target_pos, candidate.map(ranges.to_block_state_id).unwrap_or(tables.air))` (a `None`/empty candidate — §E's own edge case, faithfully preserved rather than short-circuited — places air, matching `EMPTY.createLegacyBlock()`'s own behavior). **(4)** if the newly-written state is lava (freshly placed, not via reaction B): immediately run the contact-conversion scan (§I(A)) at `target_pos` — the `onPlace`-equivalent trigger.
...
**This blueprint's own, project-added `willTickThisTick`-equivalent guard** (`08-redstone-ticking.md` §3.4 describes the same-tick duplicate-schedule check diode/torch/tripwire/etc. code uses; vanilla's own fluid path applies no such guard at any of its own re-arm sites — `LiquidBlock.neighborChanged`, `LiquidBlock.updateShape`, and `SimpleWaterloggedBlock.placeLiquid` all schedule unconditionally, and vanilla's `willTickThisTick` is called only from redstone-component code, never from fluid code) is **tighter** than M3-B01's own `is_fluid_tick_pending` ("any entry queued, due or not" — M3-B01's own doc comment explicitly names this coarseness and defers the tighter guard to "whichever future blueprint needs [it]"). This blueprint adds that guard for fluids specifically, as its own design decision rather than a vanilla behavior: `ScheduledTickQueue` gains a new private per-queue field tracking exactly the position set returned by the *most recent* `drain_due_fluid_ticks` call, exposed via a new `is_fluid_tick_in_current_batch(pos) -> bool` method (an additive, non-breaking extension of M3-B01's own already-shipped `scheduled_tick.rs`). This guard is applied **only** at the three re-arm call sites this blueprint itself designates for guarding (`on_neighbor_changed`, `on_shape_update`, and the waterlog placement re-arm in §K step 2) — **never** at `on_scheduled_tick`'s own unconditional self-reschedule after a state change (§L), which would incorrectly suppress every fluid's own re-tick, since that entry is *itself* the very batch member the guard would otherwise match against.
...
- An empty shape at the target position makes a wall passable to fluid flow only when the source position's shape is also empty; a full-cube source still blocks flow into an empty target, and a full-cube target is rejected outright regardless of the source shape.
...
- Lava+water contact conversion (reaction A) is triggered synchronously from a lava cell being placed or a neighbor changing; a shape update never triggers it, since vanilla's own shape-update hook only re-arms a fluid tick.
...
- Placing a fluid into a waterloggable container via spread schedules a new fluid tick at that same position; in vanilla this re-arm belongs to the container's own `placeLiquid` implementation, not to `FlowingFluid.spreadTo`, which schedules nothing at all.
...
/// a hand-constructed shape with no `BlockWorldAccess`/registry involved: `false` if
/// `shape.is_empty()`; else compute the shape's own union bounding box across every entry in
/// `shape.boxes()` (min of mins, max of maxes per axis, read directly off `Aabb`'s already-public
/// `min`/`max: Vec3` fields — `rc-physics` itself gains no new `bounds()` method); `true` iff that
/// bounding box's mean edge length (`(x_extent + y_extent + z_extent) / 3.0`) is at least
/// `0.7291666666666666`, **or** its Y-extent alone is at least `1.0`.
...
/// directly testable against a hand-constructed shape: `true` iff some box in `shape.boxes()`
/// fully spans `[0,1]` on both axes orthogonal to `dir` **and** touches the block boundary on
/// `dir`'s own axis (the side `dir` points toward). Exact — not an approximation — for every
/// shape `tier1_shape_table()` produces (Context §F's own worked shape-reference table); only the
/// fully general multi-box face-union merge remains out of scope.
...
/// `tier1_shape_table()` — real vanilla's own *support* shape default, verified to equal the
/// collision shape for every block in this blueprint's own tier-1/tier-2 set — and delegates to
/// `is_face_sturdy_shape`. `dir` is never `Up` in any call this blueprint makes (the `Up` branch
/// vanilla defines is dead here).
pub fn is_solid_face(world: &dyn BlockWorldAccess, tables: &FluidTables, kind: FluidKind, pos: BlockPos, dir: Direction) -> bool;
...
/// in `waterlog` (a container, regardless of shape — no `can_place_liquid` check here, matching
/// vanilla's own bare `instanceof`); else `false` when `blocks_motion(pos)`; else
/// `!deny_hold_fluid.contains(pos)` — vanilla's own fixed denylist (doors, signs, ladder, sugar
/// cane, bubble column, nether/end portal, end gateway, structure void), modeled as the
/// caller-supplied `deny_hold_fluid` range list, empty by default since none of that content is
/// placeable yet.
pub fn can_hold_any_fluid(world: &dyn BlockWorldAccess, tables: &FluidTables, waterlog: &WaterloggableRegistry, pos: BlockPos) -> bool;
/// Real vanilla `canHoldSpecificFluid` (Context §F): `LiquidBlockContainer` at target ⇒ delegate
/// to `can_place_liquid`, else `true` — carries no denylist of its own, matching vanilla exactly
/// now that `can_hold_any_fluid` above carries the denylist separately.
pub fn can_hold_specific_fluid(world: &dyn BlockWorldAccess, waterlog: &WaterloggableRegistry, pos: BlockPos, kind: FluidKind) -> bool;
/// `!is_source_of(target, kind) && can_hold_any_fluid(world, tables, waterlog, target_pos) &&
/// can_pass_through_wall(...)`.
pub fn can_maybe_pass_through(world: &dyn BlockWorldAccess, tables: &FluidTables, waterlog: &WaterloggableRegistry, source_pos: BlockPos, target_pos: BlockPos, dir: Direction, kind: FluidKind) -> bool;
...
/// naming, restated precisely in doc comments at the call sites instead of renamed here. The
/// "could structurally hold this kind at all" half is real vanilla's own `canHoldFluid`
/// combinator (`can_hold_any_fluid(below) && can_hold_specific_fluid(below, kind)`), computed
/// inline here since it has exactly one caller in this blueprint and is not exposed as its own
/// function.
...
6. `is_solid_shape_pins_the_mean_edge_length_and_y_extent_thresholds` (two sub-cases, pure — calls `occlusion::is_solid_shape` directly against a hand-built `VoxelShape`, no `FakeWorld` involved) — (a) the chest-shaped box `[0.0625,0.9375]x[0,0.875]x[0.0625,0.9375]` (extents 0.875/0.875/0.875, mean edge length exactly `0.875`, at or above `0.7291666666666666`, Y-extent `0.875` below `1.0`): asserted `true` — solid via the mean-edge branch alone, despite not being a full cube; (b) the redstone-wire-shaped box `[0,1]x[0,0.0625]x[0,1]` (extents 1.0/0.0625/1.0, mean edge length exactly `0.6875`, below the threshold, Y-extent also below `1.0`): asserted `false` — both branches fail. Both shapes' extents and the mean-edge arithmetic are stated directly in the test's own doc comment so the pinned numbers are hand-verifiable without re-deriving them, reusing (not reinventing) the exact box literals `crates/physics/src/shapes.rs`'s own `chest_shape`/`wire_shape` already define.
7. `force_solid_override_wins_before_the_shape_geometry_test` (two sub-cases, `FakeWorld`-backed, via `occlusion::is_solid(world, tables, pos)`) — (a) this suite's own ordinary "solid stone terrain" id (unregistered in `tier1_shape_table()`, so it already reads solid by geometry alone via the default-full-cube fallback) additionally placed in `tables.force_solid_off`: asserted `false` — directly contradicting what the geometry test alone would say, proving the override branch is actually consulted and wins; (b) this suite's own "air" id (empty shape, non-solid by geometry) placed in `tables.force_solid_on`: asserted `true` — the mirror case, proving the override wins in both directions, not just the one the geometry test happens to agree with.
...
6. `ice_exception_list_overrides_the_real_sturdiness_test_for_solid_face_purposes` — a full-cube block id (sturdy on every face by the real per-face test, not merely by the retired `is_full_cube` approximation) placed in `tables.solid_face_exceptions`; assert `is_solid_face` returns `false` for it despite `is_face_sturdy_shape` returning `true` for the identical shape — proving the override mechanism works even though no real ice content exists yet.
7. `is_face_sturdy_shape_matches_the_tier1_tier2_shape_reference_table` (pure — calls `occlusion::is_face_sturdy_shape` directly against hand-built `VoxelShape`s reusing the exact box literals `crates/physics/src/shapes.rs` already defines for these blocks, no `FakeWorld`/`BlockWorldAccess` involved) — asserts, for every horizontal `Direction`: the full-cube shape (terrain/furnace/piston) is sturdy on every direction; the chest shape (inset on every side, height `0.875`) is **not** sturdy on any horizontal direction despite `is_solid_shape` reading `true` for the identical shape; the hopper shape's two-box union is **not** sturdy on any horizontal direction either, even though its own union bounding box clears `is_solid`'s threshold — the same chest/hopper divergence Context §F's own reference table states in prose, pinned here directly against the real per-face sturdiness test rather than the retired `is_full_cube` approximation.
...
3. **`fluid/occlusion.rs`.** Depends on `rc_physics::shapes::tier1_shape_table()`/`VoxelShape`. Implement the two shape-occlusion fast paths (Context §F) plus `can_hold_any_fluid`/`can_hold_specific_fluid`'s real vanilla split and the waterlog-delegation checks. Implement `is_solid_shape`/`is_face_sturdy_shape` first, as pure functions operating on a `&VoxelShape` alone (no `BlockWorldAccess`, no registry), then `is_solid`/`blocks_motion`/`is_solid_face` as the thin `tables.force_solid_on`/`force_solid_off`-checking, `tier1_shape_table()`-resolving wrappers around them (Context §F, including its own hand-verified tier-1/tier-2 shape-reference table). `is_hole` composes `can_pass_through_wall(Down, ..)` with the "same kind already there, or could hold this kind at all" check (the latter is vanilla's own `canHoldFluid` combinator, `can_hold_any_fluid && can_hold_specific_fluid`, computed inline). Observable: `fluid_source_creation_drain.rs` tests 6–7 (`is_solid_shape`'s threshold pinning, the force-solid-override precedence) and `fluid_flow_field.rs` test 7 (`is_face_sturdy_shape` against the tier-1/tier-2 shape reference table) pass directly; the remaining occlusion primitives are exercised indirectly by step 4's tests.
...
5. **`fluid/reaction.rs`.** `check_lava_water_contact` implements the single interleaved `LAVA_CONTACT_ORDER` scan exactly as Context §I(A) restates it (at each of the 5 positions, the water test then the soul-soil-gated blue-ice test, in that order, before advancing to the next position). Observable: `fluid_lava_water_matrix.rs` tests 1–6, 9 pass.
