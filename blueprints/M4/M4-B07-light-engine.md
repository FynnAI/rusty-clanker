...
`rc_chunk_storage::HeightmapSet`/`HeightmapKind::WorldSurface` (WORLD-D5, M2-B01): `world_y(HeightmapKind::WorldSurface, x, z) -> i32` returns (highest non-air block's Y) + 1 for that column, using a `!is_air` predicate alone — opacity and transparency play no role, so a transparent non-air block such as glass or water is itself counted as the topmost block and the recorded Y sits one block above it, never at it — this is the **sole** heightmap this blueprint reads (WORLD-D7's own text: "skylight's sources are precomputed externally via `HeightmapSet::WORLD_SURFACE`"). `HeightmapSet::new_uniform(first_air_world_y)` — the constructor a superflat filler world's chunks are expected to be seeded with (§8 below).
...
Two independent channels — **sky light** and **block light** — share one propagator algorithm, run separately (no shared queue, no shared state between the two channels; a channel is selected by a `LightChannel { Sky, Block }` parameter threaded through every function below). Each channel maintains an **increase** queue and a **decrease** queue (§4). Vanilla itself keeps exactly one increase queue and one decrease queue per channel for the whole level — never per chunk — with each entry carrying an absolute packed block position; this blueprint's own per-chunk queue split (`ChannelState`, §4, one pair attached to every chunk's own `LightPropagatorState`) is this blueprint's own WORLD-D9 parallelization design, not a restatement of a vanilla fact — it is the concrete mechanism Stage 8's bulk-synchronous-parallel rounds (§8) operate over. The algorithm is *push*: a queued item names a position whose value just became authoritative (`from_level`), and processing it computes each of up to 6 neighbors' new target level and pushes further work only when a neighbor's value must actually change — never the reverse ("pull from neighbors") shape.
...
**Full vs. partial updates**: research doc §3.12 documents vanilla's own "border-only broadcast" optimization — "border" is a *recipient* filter, not a section filter: vanilla always sends every section whose light actually changed that tick (no notion of border-adjacency among the sections themselves), but restricts *who* receives that packet on a later in-place change to only the players for whom the chunk sits on the edge of their own tracked/view-distance area, relying on a bit-identical client-side local relight to fill in the rest for every other viewer (a chunk's own initial send, by contrast, always goes to its loading player with every section's full light state, regardless of border status) — and its own §8 "Notes for Rusty Clanker" flags this optimization as **load-bearing on Phase-2 client parity that does not exist yet** ("If Phase 2's client does not implement a bit-identical local `LightEngine`, the server's border-only broadcast strategy will produce visibly wrong light"). Since Phase 2 does not exist in this project yet (`CLAUDE.md`: "Phase 1... must reach a stable, proven state before Phase 2... begins"), this blueprint makes a deliberate, conservative, documented deviation: **`build_update_light_payload` always encodes every tracked section**, for both the initial chunk send and every subsequent light-changing update — never only the subset of sections that actually changed that tick. This can only ever send *more* bytes than vanilla would for a mid-game update, never wrong light data to a connected client (vanilla-compatible or otherwise) — a strictly safe over-approximation, not a parity gap, and is explicitly the correct interim choice per the same "documented, bounded, justified" standard `CLAUDE.md`'s binding principles require of any parity deviation. A future client-aware blueprint may add the changed-section-only encoding once Phase 2's own local relighting exists (and, separately, a future networking blueprint may add vanilla's own border-only *recipient* restriction — a chunk-holder/dispatch-level concern outside this function's own scope, since `build_update_light_payload` has no notion of which players are receiving its output); this blueprint's own function signature does not need to change when the section-content optimization happens (the *caller* simply starts calling it with a section subset — restated as an explicit non-goal in Constraints (g), not implemented here).
...
- Vanilla's block-index formula within one section is `block_index(x, local_y, z) = (local_y << 8) bitwise-or (z << 4) bitwise-or x` (4096 entries per section, vanilla's own axis order).
...
- The `WorldSurface` heightmap value for a column is (highest non-air block's Y) + 1, using a `!is_air` predicate only — opacity and transparency play no role, so a transparent non-air block such as glass or water is itself counted as the topmost block and the recorded Y sits one block above it.
...
- Vanilla maintains exactly one increase queue and one decrease queue per light channel for the whole level (never per chunk), with each queue entry carrying an absolute packed block position; the per-chunk queue split is this blueprint's own WORLD-D9 parallelization design, not a vanilla fact.
...
- At protocol 776, the `Level Chunk with Light` packet's id is `0x2D`.
