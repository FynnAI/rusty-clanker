...
4. **Concrete goal/sensor/behavior bodies** M4-B03 left to "implementer's freedom" (its own Deliverables state only the `Goal`/`Sensor`/`Behavior` trait surface and a priority *table*, never a concrete struct body) are pinned here, precisely enough to drive and assert against, reusing M4-B03's own already-given `can_use` conditions verbatim:
   ...
   ```
   Cow's `PanicGoal` (`can_use: ctx.hurt_by.is_some()`, flees at `1.5×` `MOVEMENT_SPEED` away from `hurt_by`'s own last-known position, mirroring M4-B03's own already-given Villager `FleeFromHostile` speed multiplier exactly) and Villager's `HurtBySensor` (`tick`: `if let Some(id) = ctx.hurt_by { brain.set(MemoryModuleType::HurtBy, id, Some(HURT_BY_MEMORY_TTL_TICKS)); brain.set(MemoryModuleType::HurtByEntity, id, Some(HURT_BY_MEMORY_TTL_TICKS)); }`) follow the identical pattern — restated once, not per-kind, since both are direct instantiations of the same `hurt_by`-gated shape M4-B03's own priority/activity tables already name.
...
impl ScenarioWorld {
    ...
    /// stated resolution of an ordering M4-B03's own text leaves unpinned), (3) `brain`
    /// (Villager only), (4) navigation+movement-intent — then this blueprint's own bounded,
...
| 2 | `zombie_refuses_a_four_block_drop` | Zombie | RON: a 4-block-deep pit directly on the shortest straight line to the target, with a walk-around route available | No two consecutive nodes in the returned path differ by more than 3 in `y` (the `Y-1..=Y-3` bounded descent scan, M4-B03 §F) — the honest, reframed form of "refuses an over-deep drop," matching what this engine's pathfinding actually implements rather than an unbuilt "avoid-fall" flag (Context, this blueprint's own scope correction) | Exact |
...
| 7 | `zombie_aggros_the_entity_that_hurt_it` | Zombie | Player proxy far outside `FOLLOW_RANGE` (`100.0` blocks — normally never targeted, scenario 3); `recent_damage.0 = Some(player_id)` injected once, simulating a hit landing | `current_target == Some(player_id)` on the very next tick after the pulse (`HurtByTargetGoal`, Context Part C) — "mob aggro... on damage," including the vanilla-consistent "aggro overrides ordinary range-based targeting" bound | Exact, next-tick latency |
...
| 10 | `cooldown_timed_hits_on_an_armored_target` | Zombie (real loopback connection, B05's own established pattern — `HardcodedWorld`, not `ScenarioWorld`) | `world.debug_spawn_mob(Zombie, ..)`; `world.debug_override_attribute(zombie_net_id, rc_registries::generated_v776::registries::attribute::ARMOR, 10.0)` (the retyped signature, Context Part B) (armor toughness left at `0.0`, so `real_armor = clamp(10.0 - dmg/2.0, 2.0, 20.0)`); three `Interact{Attack}` sends at simulated ticks `T0` (full charge, `ticker >= 5`), `T0+3` (undercharged, still within the `20`-tick invulnerability top-up window), `T0+3+5` (full charge again) | Hand-derived per M4-B05's own exact formulas (Context, restated once here): hit 1 — `charge=1.0`, `base=2.0*1.0=2.0` (unenchanted `AttackDamage=2.0` default player attribute), `armor_fraction=clamp((10.0-2.0/2.0)/25.0,0,1)=0.36`, `dealt=2.0*0.64=1.28`; hit 2 (undercharged, `ticker=3`, `charge=clamp(3.5/5,0,1)=0.7`) — top-up branch (`invulnerable_time>10`): `raw=(0.2+0.7²*0.8)*2.0=1.184`, `armor_fraction` recomputed against this hit's own `raw` — `dealt2_gross=1.184*(1-clamp((10-1.184/2)/25,0,1))=1.184*0.6237≈0.7385`; since `dealt2_gross <= last_hurt(1.28)`: `NoOp`, no further health change; hit 3 (`T0+8`, `invulnerable_time` has decremented below `10` by then, full charge) deals fresh full damage identically to hit 1's own formula, `≈1.28` again. Health sequence asserted to `1e-4`: `20.0 -> 18.72 -> 18.72 (unchanged) -> 17.44` | Exact (all inputs deterministic, formulas fully pinned by M4-B05) |
| 11 | `charged_critical_exceeds_uncharged_by_the_documented_envelope` | Zombie (real loopback connection) | Hit A: thrown at `ticker=0` (`charge=0.2² ... ` — restated exactly: `charge_scale=clamp(0.5/5,0,1)=0.1`, `base_damage_scale_factor=0.2+0.01*0.8=0.208`); Hit B: thrown after waiting `5` ticks idle then jumping (airborne, `fall_distance>0`, `!on_ground`, `!sprinting`, `!in_water` — critical-eligible per M4-B05's own `can_critical_attack`), full charge (`charge_scale=1.0`, `factor=1.0`, `×1.5` critical) | `damage(B) / damage(A) >= 7.0` — hand-derived floor: `(1.0 * 1.5) / 0.208 ≈ 7.212`, this blueprint's own explicit envelope threshold set at `7.0` for headroom against the two hits' own independent armor-formula rounding | **Genuine tolerance band** (`>=`, not `==`) — the one scenario in this suite stated as a ratio bound rather than an exact value, matching the task's own explicit "explicit tolerance bands" requirement most directly |
