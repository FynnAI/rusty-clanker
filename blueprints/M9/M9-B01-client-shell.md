...
| Crates touched | `rusty-clanker-client` (`crates/client/`) only — all new content; root `Cargo.toml`'s `[workspace.dependencies]` table gains exactly one new entry (`tracing-subscriber`, named and pinned by this blueprint per `00-blueprint-spec.md`'s sanctioned "name it in the blueprint" exception, the same pattern M0-B01 used for `clap`/`xshell` and M1-B04 used for `uuid`'s extra feature — folded back into `12-workspace-structure.md`'s dependency table). No other crate is touched. |
| Estimated scope | L |
...
Cross-reference (resolved): per `09-testing-quality.md`'s TEST-D53 — added after this blueprint's own resolution above prompted it, and ratifying that resolution as the project-wide Tier-1 rule for `rc-render`/`rusty-clanker-client` rather than a one-blueprint exception — a nightly Tier-2 CI story now exists (Mesa `lavapipe`/`llvmpipe` on Linux, WARP on Windows) that will let PERF-D42's own occlusion-culling pixel-equivalence test class actually run in CI once a render graph exists to drive it. This blueprint itself still exercises Tier 1 only (M9 ships no rendering pipeline for Tier 2 to test), so nothing here changes as a result beyond this reference.
