...
WORLD-D3/D4's own text describes generated `BlockStateId(u32)`/`BiomeId(u16)` newtypes living at `crates/world/generated/<data-version>/{block_states.rs, biomes.rs}`. Neither path nor crate exists: `12-workspace-structure.md`'s WS-D13 fixes `crates/registries/generated/<protocol-version>/` (inside `rc-registries`) as the single home of generated registry/block-state tables, and that is where `M0-B07` (already merged, a Prerequisite of this blueprint) actually built them: real `BlockStateId(u32)`/`RegistryEntryId(u32)` generated tables committed at `crates/registries/generated/v776/{block_states.rs, registries.rs}`, wired into `rc-registries`' own module tree as `rc_registries::generated_v776` by `M1-B05`.

Two constraints shape how this crate handles registry ids. The hard one: `xtask lint-deps`'s CI-enforced Rule 2 (`12`'s WS-D3, restated by `M0-B01`) forbids any dependency edge, direct or transitive, between `SIM = [rc-scheduler, rc-mechanics]` and `NETRENDER = [rc-render, rc-protocol, rc-transport-inproc, rc-transport-net, rc-auth, rc-cluster, rc-proxy]`; `rc-mechanics` (in `SIM`) already depends on `rc-chunk-storage` (`M0-B01`'s edge table, via its `server-systems` feature), so this crate may never reach any type through `rc-protocol` — a hard CI failure, not a preference. The deliberate one: although `rc-chunk-storage` already depends on `rc-registries` and *could* import `rc_registries::generated_v776`'s id types, this crate's palette containers need only opaque, `Copy`, numerically-faithful ids — never a name lookup or a table — and binding the storage layer's core types to regenerated codegen output would couple every consumer of this crate to that artifact's exact type identity for zero functional gain (the generated tables also define no `u16` biome id at all — biomes there are plain `RegistryEntryId(u32)` entries).

**Resolution, binding for this blueprint:** `rc-chunk-storage` defines its own minimal, local `RegistryId` trait and two concrete newtypes, `BlockStateId(pub u32)` and `BiomeId(pub u16)` — exactly WORLD-D3/D4's own originally-specified shapes, just homed in this crate instead of a `crates/world/` that was never scaffolded. Every value is **numerically identical** to Mojang's own global registry-assigned id (WORLD-D3's own rationale: "reusing Mojang's own global IDs verbatim... makes... bit-compatible with vanilla with zero translation layer" — this crate's newtypes uphold that identical numeric contract) but is a **textually distinct Rust type** from `rc_registries::generated_v776::block_states::BlockStateId` / `...::registries::RegistryEntryId`. Bridging the two — converting a `rc_chunk_storage::BlockStateId` to/from the generated-table type — is a thin, free (`#[inline]`, single-field-copy) conversion that belongs in whichever crate legitimately consumes **both** the storage types and the generated constants (e.g. `rc-worldgen`, or a composition-root binary) — not implemented, and not needed, by this blueprint. This is restated as a standing constraint below (Constraints (d)/(e)).
