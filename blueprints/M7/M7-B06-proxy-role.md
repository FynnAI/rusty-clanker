...
**Config surface, additive to CLUSTER-D27's own table** (a second finding, mirroring M7-B01 §F's identical `node_cert`/`node_key` addition pattern): `proxy_bind: SocketAddr` (a proxy process's own QUIC listen address for players' — no, restated: this is the proxy's *outbound-dialing* local address, players connect over plain TCP per NET-D7, unrelated to QUIC) and, on a `role = "node"` process, `proxy_accept_bind: SocketAddr` (the node's second, proxy-facing QUIC listen address, distinct from `bind`). `forwarding_secret_path: PathBuf` (a 32-byte shared secret file, operator-distributed to every proxy and every node alongside `ca_cert` — Context §F; distribution/rotation is an operational concern this blueprint does not solve, exactly matching 13's own stated stance on `ca_cert` distribution).
...
/// A proxy process's own construction config. `directory_seeds`/`tls`/`forwarding_secret`
/// are this blueprint's own additive extensions to CLUSTER-D27's config table (Context
/// §D/§J.1, flagged as findings for `13`'s own next revision).
#[derive(Clone)]
