...
2. **CLUSTER-D16's takeover *algorithm*.** This blueprint builds the primitives CLUSTER-D16 depends on — `RegionLease`, epoch fencing, the health-failure signal (§F below), and `ClusterNode::propose_assign_region` as the exact call a takeover makes — but does **not** implement "on raft-detected node failure, decide which live node gets the failed node's regions and call `propose_assign_region` for each" itself. That decision (CLUSTER-D2's least-loaded placement strategy, applied out-of-band on failure) is `rc-scheduler`-adjacent (it needs each node's own tick-duration EWMA, ARCH-D19, to place fairly) and is explicitly the dependency-graph edge `cluster --> sched` that `12-workspace-structure.md`'s own Dependency Graph diagram shows but WS-D3 rule 2's prose ("`rc-scheduler`... may never appear as a dependent of, nor depend on... `rc-cluster`") appears to forbid — a genuine inconsistency between that document's prose and its own diagram, flagged here as a moderate-confidence planning-corpus discrepancy for whoever authors the rebalancer/takeover blueprint to reconcile (most plausibly: the prose rule is read too strictly, or the diagram edge belongs on a *different*, not-yet-named crate that sits between `rc-cluster` and `rc-scheduler`). **This blueprint sidesteps the question entirely**: `rc-cluster`'s own Cargo.toml, as built here, has no dependency on `rc-scheduler` at all — the edge, if it is ever added, is added by whichever future blueprint builds the rebalancer/takeover logic, not by this one.
