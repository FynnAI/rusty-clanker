...
- [ ] `.github/workflows/ci.yml` gains the three scheduled TEST-D58 jobs (`protocol-capture-oracle`, `protocol-capture-ours`, `protocol-diff`) beside `m3-acceptance`, never Tier 1.
...
TEST-D54: "Runs as its own scheduled CI tier next to `m*-acceptance`; **never** part of Tier 1." TEST-D58 fixes the shape: **three jobs per OS**, all gated `if: github.event_name == 'schedule' || github.event_name == 'workflow_dispatch'`, full `{ubuntu-24.04, windows-2025}` matrix.

- `protocol-capture-oracle`: needs a real Java 25+ runtime (`actions/setup-java@v4`) and `RC_ORACLE_EULA_ACCEPTED: "1"` in its job env (the same one-time-per-CI-environment consent `m3-acceptance` already grants itself); runs `cargo run -p xtask -- protocol-diff --side oracle --accept-eula --capture-deadline-secs 5400`; uploads `target/verify/protocol-diff-oracle.postcard`, `target/verify/protocol-diff-timings.json` and `target/verify/protocol-diff.json` as artifact `protocol-capture-oracle-<os>`. A live oracle run every time — never a cached capture (TEST-D48); the local `corpus/protocol-diff/oracle.postcard` cache is a developer-iteration convenience that CI never reads.
- `protocol-capture-ours`: builds `rusty-clanker-server --release --no-default-features --features monolithic` (`RUSTC_BOOTSTRAP: "1"`), runs `cargo run -p xtask -- protocol-diff --side ours --server-bin target/release/rusty-clanker-server --capture-deadline-secs 5400`; uploads `target/verify/protocol-diff-ours.postcard` plus the timings and report JSON as `protocol-capture-ours-<os>`.
- `protocol-diff`: `needs:` both capture jobs of its OS; downloads both artifacts; runs `cargo run -p xtask -- protocol-diff --diff-only --oracle-capture <path> --ours-capture <path>` (no Java, no server build — the diff is milliseconds); uploads `target/verify/protocol-diff.json`. This job's conclusion is the one PLAN-D9(a) reads.

Both capture jobs run in parallel, so the tier's wall-clock is the slower capture, not their sum; each side's own per-step timings (§3.11) are uploaded even when the side exceeds its budget. The `rc-paritybot` lint+test steps stay where they are today (in `m*-acceptance`), not repeated in the capture jobs.

### 3.11 Progress and timing instrumentation

`protocol_diff_runner` prints one stderr line at the start of a side (`begin <side> steps=<n> contraptions=<m>`), one per completed scripted step or contraption (`done <id> in <ms> ms`) and one at the end (`finished <side> total_ms=<ms>`). `xtask protocol-diff` parses them from the drained stderr — on success or on timeout, since `xtask::process::spawn_drained`'s timeout carries the output captured so far — into `target/verify/protocol-diff-timings.json` (`{"oracle": {"completed": [{"id", "ms"}…], "total_ms", "timed_out", "deadline_secs"}, "ours": {…}}`), and a timed-out side's case detail names how far it got ("completed K of M, last: <id>"). `--capture-deadline-secs <n>` sets both sides' budget (absent: 3300 s oracle / 3000 s ours, the pre-TEST-D58 values).

### 3.12 Known-divergence register (TEST-D59)

`crates/testing/gametest/corpus/protocol-diff/known-divergences.ron` — a RON list of entries `(steps: "<step id or glob such as session/place/*>", packet: "<namespaced clientbound name, e.g. minecraft:set_time>", class: Missing | Body | Timer, closes_with: Option<"<milestone blueprint id or ledger entry title>">, expires: Option<"<milestone id>">)` — `Missing`/`Body` entries require both options, `Timer` entries carry neither (presence and count ignored in both directions for good, since two independently ticking servers never align a timer-driven packet to the same step window; the first real run showed exactly this: `set_time`, `keep_alive`, item-drop `move_entity_*`/`set_entity_motion` differing between the two runners for the same step), covered by the gametest corpus manifest (TEST-D47) and protected by the path guard (governance changesets only). `xtask protocol-diff`'s diff step resolves every mismatch against the register: a match becomes a `pass` case whose detail reads `known (<closes_with>, expires <milestone>)`; an unmatched mismatch stays a `fail`; an entry whose `expires` milestone has a `*-COMPLETION-REPORT.md` under `blueprints/` fails the run as `expired known divergence`. The report JSON lists registered, unregistered and expired counts per step. Packet names come from the datagen `reports/packets.json` mapping the normalizer already uses for `packet_name`; an entry naming an unknown packet is a parse error. Every entry the first real run needs is written by the planning role with the M4/M5 blueprint (or ledger item) that closes it — the register is the backlog, visible and shrinking.
...
No tick-freeze/barrier dance here, unlike `corpus_capture.rs`: this pass captures the **real, unfrozen** clientbound stream a real client would see — freezing time would itself change the wire shape (packets a client never actually receives in that order/timing). `spec.max_ticks` bounds a generous real-time wait (mirrors `placement_capture.rs`'s own `ACTION_SETTLE_TICKS`/`INTERACTION_SETTLE_TICKS` idiom) rather than a tick-exact barrier. Time keeps running, but the oracle's background world simulation outside the M1–M3 surface does not: right after `launch_oracle_server` reports ready, `protocol_diff_runner`'s oracle side sends `gamerule spawn_mobs false`, `gamerule random_tick_speed 0` and `gamerule advance_weather false` through `send_console_command` and confirms each with `verify_setup_commands_accepted` (the same acknowledgement strings `corpus_capture.rs` uses) — natural mob spawning, random-tick vegetation and weather changes are nondeterministic in position and count, so their packets can be neither compared nor honestly registered (a `Missing` entry keyed on `block_update` for a step would mask a real gap in that step), whereas a scripted action on an otherwise quiet world compares exactly; `set_time` and the other clock-driven packets stay `Timer`-class register entries.

**Mismatch buckets are keyed by connection state and packet id, never by the raw id alone.** A step whose capture crosses Login → Configuration → Play (`session/disconnect_reconnect`, `session/observe_chunk`) carries packets whose raw ids collide across states (Configuration `update_enabled_features` and Play `chunk_batch_start` are both id 12); the diff resolves each packet's state from the connection's own transitions inside the step, buckets mismatches by `(state, id)`, names each bucket from that state's table, and the register matches on that name — so a `Missing`/`Body` entry for a Play packet can never be shadowed by a Configuration packet sharing its id.
...
The three TEST-D58 jobs of §3.10 — `protocol-capture-oracle`, `protocol-capture-ours`, `protocol-diff` (`needs:` the two captures of its OS, `actions/download-artifact@v4` for both) — placed after `m3-acceptance`, each with the `if:` gate, matrix, toolchain and cache steps `m3-acceptance` already uses; the oracle job alone carries `actions/setup-java@v4` (`temurin`/`25`) and `RC_ORACLE_EULA_ACCEPTED: "1"`, the ours job alone the release server build. Artifacts as listed in §3.10, uploaded `if: always()`.
