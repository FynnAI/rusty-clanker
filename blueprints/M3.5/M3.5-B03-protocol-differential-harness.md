...
1. §3.8's per-packet masked-field list (which named field(s) each listed packet type carries that this harness must mask), decoded structurally per the typed-decoder rule -- no byte-offset claims are made.
