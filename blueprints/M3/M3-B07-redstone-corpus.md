...
4. **Gamerules.** `gamerule doDaylightCycle false`, `gamerule doWeatherCycle false`, `gamerule randomTickSpeed 0`, `gamerule doMobSpawning false` — eliminates every non-redstone source of block-state change from the capture window, so every observed `BlockObservation` is attributable to this contraption's own mechanism, not incidental world noise.
...
| 19 | `daylight_sensor_clock_stub` | Clock | Daylight sensor's 20-tick recompute cadence (§3.10) — gamerule `doDaylightCycle` re-enabled just for this one contraption's capture window |
