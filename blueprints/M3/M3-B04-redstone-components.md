...
`should_prioritize(pos)` (research §3.6, restated as this blueprint's own precise boolean formula from the research corpus's English description — "the diode directly behind it... is itself a diode whose own `FACING` does *not* point back, i.e. it isn't feeding straight through"): let `behind = facing(pos).opposite().apply(pos)` (the neighbor `pos`'s own output flows into — see **Signal output** below); `should_prioritize(pos) = registry.resolve(state_at(behind)).is_diode() && facing(behind) != facing(pos).opposite()` — `behind`'s own `FACING` "points back" at `pos` (its own output flows straight back into `pos`'s input, the two diodes facing directly at each other head-to-head) exactly when `facing(behind) == facing(pos).opposite()`; that is the sole non-prioritized case. A same-facing behind-diode (the ordinary same-direction daisy chain, where `behind`'s own input reads straight from `pos`'s own output) is instead prioritized, matching every other behind-diode orientation including the perpendicular-chain case research names.

**Signal output** (ASSET-D18(f) research verdict — corrected from an earlier drafting error that had this inverted): `FACING` points toward the repeater's own INPUT side; output flows out the *opposite* side ("a repeater fires away from you" — placement sets `FACING = playerLookDirection.opposite()`, so output flows away from the placer). `weak_signal_toward(pos, towards) = if powered(pos) && towards == facing(pos).opposite() { 15 } else { 0 }`; `direct_signal_toward` identical (a repeater gives strong output out that side too, enabling QC through a block placed directly against it — matching comparator's own symmetric behavior, Context §G).
...
**Modes**: `ComparatorMode::Compare | Subtract`, stored per-position, toggled by the block-use interaction MECH-D82 added as an M3 field-report changeset (PLAN-D10; when this blueprint was written M3 had no item-use handling; this blueprint's tests set the mode directly via `ComparatorBehavior::set_mode` — a test/composition-root-only setter, not part of the `BlockBehavior`/`RedstoneSignalSource` public contract).
...
**Signal output** (ASSET-D18(f) research verdict — corrected from an earlier drafting error that had this inverted, symmetric with repeater's own correction above): `FACING` points toward the comparator's own INPUT side; output flows out the *opposite* side. `weak_signal_toward(pos, towards) = if towards == facing(pos).opposite() { stored_output(pos) } else { 0 }`; `direct_signal_toward` identical (a comparator gives strong output out that side too, matching repeater's own symmetric behavior — enabling QC through a block placed directly against a comparator).

### H. Tier-1 scope: lever/button/pressure-plate are out of this blueprint (the lever was pulled into M3 by PLAN-D10 and closed as an M3 field-report changeset; button and pressure plate are M4-B10)
...
6. `repeater_output_flows_out_the_side_opposite_facing` — a real repeater driven through its own `on_neighbor_changed`/`on_scheduled_tick` cycle to `powered(pos) == true`; assert `weak_signal_toward`/`direct_signal_toward` answer nonzero only toward `facing(pos).opposite()`, never toward `facing(pos)` (**Signal output** above, direct formula-level coverage — this exact formula previously had zero test coverage in either direction, which is how its own inversion went unnoticed).
7. `repeater_chain_relays_signal_end_to_end` — the genuine end-to-end regression: a straight 3-repeater daisy chain (all same facing), fed by a fixed source touching only the first repeater's own input face, driven through the real per-tick driver (`stage4::run_scheduled_phase`); assert every repeater in the chain ends up `powered(pos) == true`, i.e. the signal actually reaches the far end — not merely that each repeater's own state machine flips in isolation.
8. `repeater_should_prioritize_perpendicular_chain` — a second repeater directly behind (`facing.opposite().apply(pos)`), facing **perpendicular** to the first (i.e. `facing(behind)` is neither `facing(pos)` nor `facing(pos).opposite()`); assert `should_prioritize(pos) == true` and a subsequent `checkTickOnNeighbor`-triggered schedule uses `TickPriority::ExtremelyHigh`.
9. `repeater_same_facing_chain_is_prioritized` — identical setup but the behind repeater's `facing == facing(pos)` (the ordinary same-direction daisy chain, where `behind`'s own input reads straight from `pos`'s own output); assert `should_prioritize(pos) == true` (**`should_prioritize(pos)`** above — a same-facing behind-diode is prioritized, not the non-prioritized case an earlier drafting error had this test asserting).
10. `repeater_head_to_head_chain_is_not_prioritized` — identical setup but the behind repeater's `facing == facing(pos).opposite()` (its own output flows straight back into `pos`, the two diodes facing directly at each other head-to-head); assert `should_prioritize(pos) == false` — the genuine non-prioritized case.
11. `repeater_input_reads_wire_power_directly` — a wire in front at power `7`, and a separate plain-conductor path that would otherwise read a lower plain signal; assert `base_diode_input_signal` returns `7` (the wire-power special case, Context §F/research §3.6).
...
8. `comparator_output_flows_out_the_side_opposite_facing` — a comparator with a nonzero stored `output`; assert `weak_signal_toward`/`direct_signal_toward` answer nonzero only toward `facing(pos).opposite()`, never toward `facing(pos)` (**Signal output** above, direct formula-level coverage — mirrors the repeater's own equivalent test; this exact formula previously had zero test coverage in either direction).
