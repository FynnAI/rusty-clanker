...
**Emission timing — same-tick, not next-tick.** `PistonBehavior::on_neighbor_changed` runs during Stage 4's **scheduled phase** (B01's `system_scheduled_phase`, registered `order_tag` 0), *before* the block-event sub-phase (`order_tag` 1) runs in that same tick. `BlockEventQueue::emit` (B01, corrected to MECH-D9's verified single live re-entrant queue) appends to the one live FIFO the block-event sub-phase drains with `pop_next` until empty. **A block event emitted from `on_neighbor_changed` is therefore processed in the block-event sub-phase of the *same* tick it was emitted** — and so is an event emitted *by a block-event handler during that handler's own processing* (from inside `PistonBehavior::on_block_event` itself): the same pass's drain loop picks it up, per MECH-D9's re-entrant same-tick cascade semantics (Context, "zero-tick" paragraph below, addresses the one case where this matters).
