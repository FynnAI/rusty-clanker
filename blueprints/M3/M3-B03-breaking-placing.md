...
- **The reach check** — `within_reach`'s straight-line-Euclidean-from-a-fixed-position body is replaced by `mining::raycast_reach` (real per-player `PlayerMotion.position`, via a voxel raycast against `rc-physics`). `EYE_HEIGHT`/`eye_position(BlockPos)` (M2-B07) are replaced by M3-B02's own `eye_position(Vec3) -> Vec3` (already shipped, unused by any code path before this blueprint per M3-B02's own Interfaces note — this blueprint is the "future M3 block-placement/breaking blueprint" that note names).
...
### Reach validation — superseded to a real per-player-position voxel raycast (MECH-D62, full text)

M2-B07 explicitly deferred MECH-D62's "raycast line-of-sight check against `rc-physics`'s own voxel shapes" because `rc-physics` did not exist. It exists now (M3-B02). This blueprint implements it, adding one new file to `rc-physics` (additive, non-breaking — `rc-physics`'s public API gains a function, nothing already-shipped changes):
...
**Reach-check call site** (`mining::raycast_reach`): `origin = eye_position(motion.position)` (M3-B02's own function, real per-player position — supersedes M2-B07's `eye_position(SPAWN_POSITION)`), `direction` derived from the player's own `(yaw, pitch)` via the SAME `mth_sin`/`mth_cos`-based look-vector construction `01`'s/M3-B02's own `get_input_vector` already establishes the trig convention for (Context, "Orientation from placement context" below gives the exact formula, shared by both call sites), `max_distance = BLOCK_INTERACTION_RANGE_CREATIVE` or `_SURVIVAL` (unchanged constants, kept from M2-B07, gamemode-selected via `GameModeState.instabuild`). A `cast_ray` result is accepted as "in reach" only if **both** (a) a hit exists at all within range, **and** (b) the hit's own `block_pos` equals the packet's own claimed target position (`target_position`, M2-B07's algorithm, unchanged) — this is MECH-D62's own "target hasn't already changed since the client's last-acknowledged world revision"-adjacent check in its simplest honest form: the ray must actually reach the claimed block, not just get within Euclidean range of it while looking somewhere else or through an intervening solid block. A player targeting a block that is *behind* another solid block (occluded) therefore now correctly fails reach — a real strengthening over M2-B07's own explicitly-looser Euclidean-only check, closing exactly the gap M2-B07's own Context flagged as accepted-but-undesirable at its own scope.

### Orientation from placement context — per-block rules (MECH-D62/MECH-D38's shared yaw convention)

Every look-direction-dependent placement rule below shares one horizontal-look-vector construction, reusing M3-B02's own already-established `mth_sin`/`mth_cos` yaw convention exactly (no new trig convention invented here): `look = Vec3::new(-mth_sin(yaw_rad) as f64 * cos(pitch_rad), -sin(pitch_rad), mth_cos(yaw_rad) as f64 * cos(pitch_rad))` where `yaw_rad = yaw as f64 * PI / 180.0`, `pitch_rad` likewise (`sin`/`cos` for the pitch term use ordinary `f64::sin`/`cos` — MECH-D62's own reach raycast is a server-authoritative *validation* aid, not a rendered/predicted quantity `18-float-determinism.md`'s trig-table rule binds; only the yaw-driven horizontal component reuses the table, matching M3-B02's own `get_input_vector` precedent exactly, restated here for the second call site that needs it).
...
/// Context's own shared look-vector construction, reused by both the raycast direction and
/// (a future blueprint's) any other look-driven mechanic.
...
/// AND `hit.block_pos == claimed_target`.
pub fn raycast_reach(motion: &PlayerMotion, claimed_target: BlockPos, range: f64, shapes: &dyn BlockShapeSource) -> bool;
...
3. `raycast_reach_rejects_an_occluded_target_even_within_euclidean_range` — a target block placed directly *behind* another solid block from `A`'s own spawn eye position (both within `5.0` Euclidean units, but the near block fully occludes the far one along `A`'s straight look line); `A` sends a `PlayerAction{status:0, location: <the far, occluded block>, ..}`; `A` reads `AcknowledgeBlockChange` then **nothing further** within a bounded timeout (`RejectReason::OutOfReach` — the raycast never reaches the claimed target, MECH-D62's own strengthened check over M2-B07's Euclidean-only one).
