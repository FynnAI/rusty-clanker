...
### The seam by which generated code (packet codegen and registry tables) plugs in

M0-B07 populated `crates/registries/generated/v776/{registries.rs, block_states.rs, MANIFEST.json}` (registry/block-state **id tables**, homed in `rc-registries` per WS-D13 — not packet definitions; M0-B07's own Constraints explicitly reserve packet-body codegen, and the `crates/protocol/generated/` directory, for M1). This blueprint does not touch either directory and does not implement NET-D9's packet-field-layout-spec-driven codegen (`crates/protocol/spec/*.ron` still does not exist). The seam is exactly the field-type table above: once a later blueprint defines a packet whose field is, say, a block-state id, that field's Rust type is `rc_registries::generated_v776::block_states::BlockStateId` (or a thin newtype wrapping it) — as long as that type implements `WireWrite`/`WireRead` (a one-line `impl` a later blueprint adds inside `rc-protocol`, which already depends on `rc-registries`, delegating to whatever primitive encoding the id actually needs, e.g. `#[rc(varint)]` on a wrapped `i32`), `#[derive(RcPacket)]` already accepts it with zero macro changes, per this blueprint's own "any other type" row in the mapping table above.
