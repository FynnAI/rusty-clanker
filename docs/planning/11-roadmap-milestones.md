...
- Tier-2 input components: button (auto-off scheduled tick) and pressure plate (entity-presence trigger) — the two inputs M3-B04 §H excluded alongside the lever that PLAN-D10 pulled into M3; blueprint `M4-B10`, not yet authored, authored before M4 wave 3 closes.
