...
| PLAN-D12 | **M4 closes on its three machine-checked criteria; the entity-sync backlog it leaves is a named NET-hardening changeset that precedes M5's own protocol-diff run.** The TEST-D59 register entries that expired with M4 and named M4-B01/M4-B02 as closers (re-announcing existing entities to a joining player, the player's own metadata and attribute sync, item position sync after a drop, `set_equipment`, per-tick block-change coalescing) are re-scoped to `NET hardening: entity tracking on join, self-entity sync and item sync` / `… block-change coalescing …` with `expires M5`; that changeset is an M5 entry-gate item next to PLAN-D11, landed before M5's implementation blueprints start. M4's own acceptance criteria (region transfer, hopper cadence, the scenario suite) never required these packets. | An expired register entry fails the differential harness the moment the milestone's completion report exists (TEST-D59), so the backlog needs a real owner and date rather than a silent extension; the M4 blueprints that would have owned it were written before the harness could see it. |
...
- Known limits carried into M5 (recorded in `blueprints/M4/M4-COMPLETION-REPORT.md` and the findings ledger): players are not yet `BaseEntity`/`LivingEntity` and other players are not tracked as entities in production (M4-B08's harness composition only); entities that exist before a (re)join are not re-announced, the player's own metadata/attribute changes (sprinting) are not synced, dropped items get no position sync, `set_equipment` is not sent — all re-scoped to the PLAN-D12 NET-hardening changeset; the client still receives placeholder full-bright chunk light although the server computes real light; random-tick receivers and the day/night clock have no owner yet; two `AttributeMap` types coexist (M4-B03's AI map, M4-B05's combat map, bridged); three AI–combat constants await their MECH- rows.
