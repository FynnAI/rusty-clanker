...
**Scope:**
- `rc-mechanics` (owned by `05-game-mechanics.md`) tier 1: player movement/collision, block placement/breaking, block-update propagation, core redstone components (wire, repeater, comparator, torch, piston) under ARCH-D13's sequential Stage 4.
...
**Scope:**
- `rc-mod-api`'s hook trait surface and `ComponentDescriptor` builder (wrapping ARCH-D4), `rc-mod-host`'s dylib loader (`libloading`), hook injection points into the five ARCH-D8 domain groups.
