...
| `rc-protocol` | `crates/protocol/` | Wire codec: VarInt/NBT/text-component encode-decode, the hand-authored packet field-layout spec (`crates/protocol/spec/*.ron`, NET-D9) and generated packet enums (`crates/protocol/generated/<protocol-version>/`, NET-D3/D9). Pure data/codec — no sockets, no Tokio. | server, client |
...
| `xtask` | `xtask/` | Dev-only tooling binary: NET-D9's `fetch-data`/`codegen`, plus `test`/`bench`/`lint`/`fmt-check`/`lint-deps`/`parity-check`/`fetch-corpus` (WS-D9/D10). Never shipped. | dev/CI only |
...
Note the two load-bearing absences: no arrow anywhere leads from `sched` or `mech` into `NetServer` or `ClientOnly` (WS-D3 rule 2), and `msg`'s only incoming... outgoing edge is to `core` (WS-D3 rule 3) — every network-carrying crate (`tinproc`, `tnet`, `cluster`) depends *on* `rc-messaging`, never the other way. `rc-physics` sits in `Shared` (WS-D3 rule 1 addition) since `mech` and `clientbin` depend on the identical compiled crate, never a forked copy.
...
rand_xoshiro      = "0.8.1"    # rc-worldgen, GEN-D3
...
│   ├── core/                   # rc-core
│   ├── nbt/                    # rc-nbt
│   ├── registries/             # rc-registries (generated/<protocol-version>/ inside)
│   ├── protocol-macros/        # rc-protocol-macros
│   ├── protocol/                # rc-protocol (spec/, generated/<protocol-version>/ — NET-D9 paths)
│   ├── mod-api/                 # rc-mod-api
...
- The four dependency-graph hard rules (WS-D3) every future domain doc's design must respect when it proposes new types or new cross-crate calls.
...
- Whether `rc-registries`' generated output should physically live under `crates/registries/generated/<protocol-version>/` (this document's choice) or be folded into `crates/protocol/generated/` as `02-protocol-networking.md`'s NET-D9 prose loosely suggested when it described `--reports`' packet/registry/block-state/item-ID JSON as one merged input — NET-D9 only fixed the *packet* codegen output path, not the registry-data path, so this is additive rather than contradictory, but worth a one-line reconciliation note in `02` next time it is revised.
