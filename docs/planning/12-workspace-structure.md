...
| WS-D15 | **Generated block-state property registry.** `rc-registries`’ codegen (WS-D13) grows a second generated table derived from the datagen `blocks.json` report: for every block, its full block-state id range, per-state property values (facing, delay, locked, powered, waterlogged, …), the default-state id, and the per-state `replaceable` flag — giving every consumer a real id↔properties mapping instead of the hand-maintained placeholder id tables M3 accumulated (`rc-physics` shape rows, piston/redstone registration ranges, oriented-state placement tables). Same custody rules as WS-D13: generated from a legally obtained local copy, committed as generated Rust with a manifest, never raw Mojang JSON in the repo. **Lands before M4 implementation starts** (PLAN-D9): M4’s items/entities/fluids work consumes properties pervasively, and every M3 placeholder is retired against it by an M4 changeset. | M3 proved the cost of not having this: three separate hand-synced literal-id tables, a shape lookup that misclassified every non-default orientation as a full cube, and project-invented placeholder ids awaiting reconciliation — each a defect class this table removes by construction. |
