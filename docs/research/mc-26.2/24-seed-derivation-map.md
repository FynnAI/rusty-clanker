...
- `nextDouble()` = `(top 53 bits of nextLong(), i.e. nextLong() >>> 11) * 2^-53` — the multiplier is the **exact** double value of `2^-53` (bits `0x3CA0000000000000`), used unmodified by both RNG families. The `1.110223E-16F`-with-`F`-suffix rendering that appears in some decompiled sources is a decompiler artifact (`docs/research/mc-26.2/18-float-determinism.md` §3.12, confirmed via direct `javap -c -constants` bytecode disassembly): the actual embedded constant is the double literal `1.1102230246251565E-16d`, i.e. exact `2^-53`, and there is no float-precision detour at runtime. The real transcription trap runs the other way: typing the decompiled-looking decimal text `1.110223E-16` directly as a Rust `f64` literal parses to a **different**, wrong value (bits `0x3C9FFFFFF4178DA3`) — derive the constant as an exact power of two (`f64::from_bits(0x3CA0000000000000)`, equivalently `2f64.powi(-53)`) instead of transcribing any decimal literal.
...
| Xoroshiro/legacy shared `nextFloat`/`nextDouble` constants | `5.9604645E-8F` = exact `2⁻²⁴` (bits `0x33800000`); `1.1102230246251565E-16d` = exact `2⁻⁵³` (bits `0x3CA0000000000000`) — both exact powers of two, no float-precision detour at runtime (§3.1, `18-float-determinism.md` §3.12) | `BitRandomSource`/`XoroshiroRandomSource` |
...
5. **`Xoroshiro128PlusPlus`'s state-update must use wrapping 64-bit arithmetic and Java's specific `rotl` definition.** `nextDouble()`'s scale constant is the **exact** double value of `2⁻⁵³` (bits `0x3CA0000000000000`), shared unmodified with the legacy family — there is no float-precision detour at runtime; the `1.110223E-16F` rendering some decompiled sources show is a decompiler artifact (`18-float-determinism.md` §3.12, bytecode-confirmed), not a real `float`-then-widen step. The actual trap runs the other way: typing the decompiled-looking decimal text directly as a Rust `f64` literal (`1.110223E-16` parsed as `f64`) gives a **different, wrong** value (bits `0x3C9FFFFFF4178DA3`) — derive the constant as an exact power of two (`f64::from_bits(0x3CA0000000000000)` or `2f64.powi(-53)`) instead of transcribing any decimal literal.
