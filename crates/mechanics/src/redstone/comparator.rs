...
impl BlockBehavior for ComparatorBehavior {
    ...
    fn on_neighbor_changed(&self, ctx: &mut UpdateContext, pos: BlockPos, _from: Direction) {
        ...
        // hopper is not a conductor but is `Rigid`-sturdy, and a comparator does survive on
        // one; a chest never reaches its own top boundary at all, so it is `Rigid`-sturdy on
        // neither block, and a comparator pops off one just as a wire does).
        ...
        // `is_block_tick_pending` — vanilla's comparator refuses to schedule only while its own
        // tick is collected-but-not-yet-run in *this* game tick. A tick already queued for a
        // later game tick is refused one level down instead, by `schedule_block_tick`'s own
        ...
        if mismatch && !ctx.scheduled.will_block_tick_this_tick(pos) {
