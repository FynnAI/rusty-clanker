...
fn read_level_dat_falls_back_to_dat_old_when_primary_is_corrupt() {
    ...
    std::fs::write(dir.path().join("level.dat"), b"garbage, not gzip nbt at all").unwrap();
...
fn second_open_on_the_same_world_root_fails_with_world_already_open() {
    ...
    let err = AnvilDiskBackend::open(dir.path().to_path_buf(), CompressionScheme::Zlib).unwrap_err();
    assert!(matches!(err, StorageError::WorldAlreadyOpen { .. }));
