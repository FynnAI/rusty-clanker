...
async fn torch_next_to_a_running_hopper_disables_it_and_removal_re_enables_it() {
    tokio::time::timeout(Duration::from_secs(300), async {
        ...
        // `hopper_placed_with_no_neighbor_signal_stays_enabled` geometry.
        world
            .debug_set_held_item(1, HeldItemStub::Block(PlaceableBlockKind::Hopper))
            .await;
        let hopper_pos = BlockPos::new(0, -59, 0);
        let hopper_id =
            place_and_read_id(&mut a, &mut a_acc, &mut seq, BlockPos::new(0, -60, 0), 1).await;
        assert_eq!(
