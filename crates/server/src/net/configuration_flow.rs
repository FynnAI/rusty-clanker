        ...
        tokio::select! {
            maybe_raw = inbound.recv() => {
                ...
                if raw.id == ConfigurationKeepAliveServerbound::ID {
                    ...
                    // server always sends every registry entry `has_data=false` regardless of
                    // the client's echoed response (Step 4, below), never inspecting the
                    // echoed `known_packs` content itself, so the response is decoded (a
                    // malformed body is still a fatal protocol violation) but never gates on
                    // its *content* -- any echoed list, including an empty one, is accepted
                    // and the sequence proceeds.
...
pub async fn run_configuration(
    ...
    // Step 4: registry-data sync. Every entry is unconditionally `has_data=false` (Context;
    // `RegistryDataEntryOut::data: None`) -- the 1.20.5+ synchronized-registry semantics where
    // an entry with no inline payload tells the client "use your own already-loaded built-in
    // definition for this name," correct for every entry `worldgen_registries` ever carries
    // since every one of them is a stock vanilla name a real client already ships (M1 registry-
    // sync fix; `play::world::SYNCHRONIZED_REGISTRIES`'s own doc comment has the full
    // trade-off writeup, including why no registry is special-cased with real inline NBT here
    // any more).
