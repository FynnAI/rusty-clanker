...
/// and re-exported piecemeal. The `chunk` module stays private: every acceptance test
/// that needs chunk-byte assertions writes its own test-local decode mirror instead
/// (Acceptance tests' own explicit instruction, "not a production deliverable").
pub mod packets;
mod world;

pub use connection::{PlayerProfile, enter_play};
pub use keepalive::{DisconnectReason, KeepAliveAction, KeepAliveDriver};
pub use world::{HARDCODED_REGION_ID, HardcodedWorld, PlayerMarker};
