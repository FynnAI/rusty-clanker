...
**IMPLEMENTATION — M4 complete; M5 entry gate (planning/hardening) in progress** (M0 complete 2026-08-24; M1 complete + real-client-verified 2026-08-26; M2 complete + real-client-verified 2026-08-27; M3 complete on every machine-checked criterion 2026-09-02; M3.5 hardening complete 2026-09-06 — all five criteria green, criterion 2 on the scheduled protocol-diff run 34014122282 — `blueprints/M3.5/M3.5-COMPLETION-REPORT.md`; M4 started on the user's explicit go 2026-09-03 and is complete on all three machine-checked criteria 2026-09-07 — `xtask m4-report` 13/13, `m4-acceptance` CI job green, `blueprints/M4/M4-COMPLETION-REPORT.md`; the owner play-tests the pinned `m4-final` tag/binary and every finding from it is an M4 field report, never mixed into M5; M3 field-report wave 3 (PLAN-D10) closed on `main` with the `m3-fr3-final` re-test binary, verdict pending. M5 implementation starts only on the user's explicit go; until then the M5 entry gate runs on the user's standing instruction: PLAN-D11 re-authoring of the eight flagged M5 blueprints and the PLAN-D12 NET-hardening changeset). All milestones M0–M11 are blueprint-complete; code is written strictly from the audited blueprints in `blueprints/` (per-blueprint test-first changesets per TEST-D45/D46, `Changeset-Type` trailer convention per M0-B08). Planning documents in `docs/planning/` remain the source of truth on any conflict. **Hard gate: after each milestone's acceptance criteria are green, STOP — the next milestone starts only on the user's explicit go.** Phase 1 (server) must reach a stable, proven state before Phase 2 (client) implementation begins.
...
- **Planning authority, and where findings go.** Only the planning role creates or edits planning content: decision IDs and everything under `docs/planning/` are its exclusive territory. Implementation never adds a decision, however good the idea. Implementation *will* surface such items constantly — open questions, deviations it had to ship, and factual errors it found in a blueprint — so every one of them is appended to `docs/findings-for-planning.md` (its own Purpose section defines the three categories) instead of being decided, and instead of being left to evaporate in session history. Correcting a factual error in a blueprint, verified against the ASSET-D18(f) reference, is error correction rather than planning and stays allowed — but it is recorded in that file too, since the planning documents may still describe the superseded behaviour.
...
## Agent model policy (binding, user directive 2026-09-02)
