...
**IMPLEMENTATION — milestone M4 (mechanics tier 2)** (M0 complete 2026-08-24; M1 complete + real-client-verified 2026-08-26; M2 complete + real-client-verified 2026-08-27; M3 complete on every machine-checked criterion 2026-09-02; M3.5 hardening complete 2026-09-06 — all five criteria green, criterion 2 on the scheduled protocol-diff run 34014122282 — `blueprints/M3.5/M3.5-COMPLETION-REPORT.md`; M4 started on the user's explicit go 2026-09-03 with the owner's M3 real-client sign-off deferred: the owner play-tests the pinned `m3.5-final` tag/binary, and every finding from it is an M3 field report, never mixed into M4; the first such wave — six findings, PLAN-D10 — is being closed on `main` before M4 wave 3 starts). All milestones M0–M11 are blueprint-complete; code is written strictly from the audited blueprints in `blueprints/` (per-blueprint test-first changesets per TEST-D45/D46, `Changeset-Type` trailer convention per M0-B08). Planning documents in `docs/planning/` remain the source of truth on any conflict. **Hard gate: after each milestone's acceptance criteria are green, STOP — the next milestone starts only on the user's explicit go.** Phase 1 (server) must reach a stable, proven state before Phase 2 (client) implementation begins.
